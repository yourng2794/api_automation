...
from O2oUtility import AdminProviderPayOptionBankInfo
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider payOption bank info'
        self.CaseNote= ''
        self.TestCaseID= '151'
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption bank info'+'\n')
        ...
        ReplaceIniValue('admin_provider_payOption_bank_info', 'API.payOptionId', payoptionId)
        
        self.res_log, result = AdminProviderPayOptionBankInfo(self.res_log, 'Boss', payOptionName, payoptionId)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption bank info')
