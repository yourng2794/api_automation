...
class TestCase(BaseCase):
    ...
    def run(self, copy_result):
        ...
        try:
            ...
            ReplaceIniValue('admin_partner_payment_get', 'API.parentId', partnerId)
            ReplaceIniValue('admin_partner_payment_get', 'API.currency', '{}')
            ReplaceIniValue('admin_partner_payment_get', 'API.api_response', '{"Error_code":"S01","Error_message":"Success","payment":[]}')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_partner_payment_get', True, False)
        ...
        try:
            ReplaceIniValue('admin_partner_payment_get', 'API.currency', '{"CNY":"1"}')
            ReplaceIniValue('admin_partner_payment_get', 'API.api_response', '{"Error_code":"S01","Error_message":"Success","payment":{"ALI":"1","WECHAT":"1","BOC":"1","FASTPAYMENT":"1"}}')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_partner_payment_get', True, False)
            WriteLog(print_tab+'Response data: '+ response_data)
            if(return_value):
                result_list.append('PASS')
