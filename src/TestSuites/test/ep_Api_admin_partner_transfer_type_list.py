...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST partner transfer type list'
        self.CaseNote= ''
        self.TestCaseID= '62'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner transfer type list'+'\n')
        print_tab = '\n\t\t'
        try:
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_partner_transfer_type_list', True, False)
            ...
            if(return_value):
                ...
                result_list.append('PASS')
                for json_value in json_response.get('boss').values():
                    if(json_value.get('pt_type_name') == typeName):
                        typePermissionId = json_value.get('pt_type_permissionid')
                        ReplaceIniValue('setting', 'API.typepermissionid', typePermissionId)
            ...
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_partner_transfer_type_list... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner transfer type list')
