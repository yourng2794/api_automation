...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST partner type payment get'
        self.CaseNote= ''
        self.TestCaseID= '62'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner type payment get'+'\n')
        print_tab = '\n\t\t'
        try:
            ReplaceIniValue('admin_partner_type_payment_get', 'API.currency', '{"CNY":"1"}')
            ReplaceIniValue('admin_partner_type_payment_get', 'API.api_response', '{"Error_code":"S01","Error_message":"Success","payment":{"ALI":"1","WECHAT":"1","BOC":"1","FASTPAYMENT":"1"}}')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_partner_type_payment_get', True, False)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_partner_type_payment_get... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner type payment get')
