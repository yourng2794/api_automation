...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST partner edit agent'
        self.CaseNote= ''
        self.TestCaseID= '168'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner edit'+'\n')
        print_tab = '\n\t\t'
        try:
            agentAcc = IniValue('setting', 'API.agentAcc')
            agentId = IniValue('setting', 'API.agentid')
            currency = IniValue('admin_partner_edit', 'API.currency')
            json_currency = json.loads(currency)
            temp_currency = json_currency['HKD']
            json_currency['HKD'] = '0'
            ReplaceIniValue('admin_partner_edit', 'API.currency', str(json_currency).replace("'",'"').replace(' ',''))
            ReplaceIniValue('admin_partner_edit', 'API.ptPwd', agentAcc)
            ReplaceIniValue('admin_partner_edit', 'API.ptId', agentId)
        ...
        try:
            currency = IniValue('admin_partner_edit', 'API.currency')
            json_currency = json.loads(currency)
            json_currency['HKD'] = temp_currency
            ReplaceIniValue('admin_partner_edit', 'API.currency', str(json_currency).replace("'",'"').replace(' ',''))
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_partner_edit', True, False)
            WriteLog(print_tab+'Response data: '+ response_data)
            if(return_value):
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner edit')
