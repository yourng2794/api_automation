...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Portal] API POST login exist account'
        self.CaseNote= ''
        self.TestCaseID= '19'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('bc_login', 'API.code', '')
        ReplaceIniValue('bc_login', 'API.acc', '')
        ReplaceIniValue('bc_login', 'API.pwd', '')
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Portal] API POST login exist account'+'\n')
        ...
        while(retry < 10):
            try:
                return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'bc_login_code', False, True)
...
#                 input(print_tab+'- Please enter code (4 digits): ')
                ...
                ReplaceIniValue('bc_login', 'API.acc', bc_acc)
                ReplaceIniValue('bc_login', 'API.pwd', bc_pwd)
            
                return_value2, response_time2, response_data2, self.res_log = ApiResponse(print_tab, self.res_log, 'bc_login', True, True)
...
#                 result_list.append('FAIL')
                self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on bc_login... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Portal] API POST login exist account')
