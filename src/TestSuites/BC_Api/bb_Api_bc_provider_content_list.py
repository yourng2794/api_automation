...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider content list'
        self.CaseNote= ''
        self.TestCaseID= '18'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider content list'+'\n')
        print_tab = '\n\t\t'
        try:
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'bc_provider_content_list', False, False)
...
#                 list_response = list(response_data)
#                 print(len(list_response))
                json_response = json.loads(response_data)
                for json_element in json_response:
                    print(json_element.get('providerid'), json_element.get('providername'))
#                     print(json_element)
...
#                 if((json_response.get('acc') == current_user) and (json_response.get('num') != None) and (json_response.get('wallet') != None)):
#                     player_id = json_response['playerid']
#                     ReplaceIniValue('setting', 'BCAPI.playerAcc', current_user)
#                     ReplaceIniValue('setting', 'BCAPI.playerId', player_id)
#                     self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get (player_acc, player_id): ('+current_user+', '+player_id+')')
#                 else:
#                     result_list.append('FAIL')
#                     self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get player_acc: '+json_response['acc']+' != '+current_user)
#                 self.res_log = WriteDebugLog(self.res_log, print_tab+'---')
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on bc_provider_content_list... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider content list')
