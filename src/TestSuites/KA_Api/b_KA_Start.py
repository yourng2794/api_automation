...
from O2oUtility import WriteLog, WriteDebugLog, ApiResponse, IniValue, ReplaceJsonValue
...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Game] API POST KA start'
        self.CaseNote= ''
        self.TestCaseID= '22'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Portal] POST KA start API'+'\n')
        ...
        try:
            dt = datetime.now()
            api_json = tooldir+'\\JsonFiles\\Api_Json\\ka_start.json'
            timestamp = int(dt.timestamp())
            ReplaceJsonValue(api_json, 'timestamp', str(timestamp)+'000')
            token = IniValue('setting', 'API.token')
            sessionid = IniValue('setting', 'API.sessionid')
            playerid = IniValue('setting', 'API.playerid')
            ReplaceJsonValue(api_json, 'token', token)
            ReplaceJsonValue(api_json, 'sessionid', sessionid)
            ReplaceJsonValue(api_json, 'partnerPlayerId', playerid)
            
            api_response = IniValue('ka_start', 'API.api_response')
            file = open('response.json', 'w')
            file.write(api_response)
            file.close()
            
            ReplaceJsonValue('response.json', 'sessionId', sessionid)
            ReplaceJsonValue('response.json', 'playerId', playerid)
            
            return_value, response_time, response_data, self.res_log = ApiResponse('\n\t\t', self.res_log, 'ka_start', True, False)
            if(return_value):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Portal] POST KA start API')
