...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Wallet] API POST KA credit'
        self.CaseNote= ''
        self.TestCaseID= '25'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Portal] POST KA credit API'+'\n')
        ...
        try:
            dt = datetime.now()
            api_json = tooldir+'\\JsonFiles\\Api_Json\\ka_credit.json'
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, '\n\t\t- Exception: on ka_credit... %s' % str(e))
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Portal] POST KA credit API')
