...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Terminate] API POST KA revoke'
        self.CaseNote= ''
        self.TestCaseID= '27'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Portal] POST KA revoke API'+'\n')
        ...
        try:
            dt = datetime.now()
            api_json = tooldir+'\\JsonFiles\\Api_Json\\ka_revoke.json'
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, '\n\t\t- Exception: on ka_revoke... %s' % str(e))
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Portal] POST KA revoke API')
