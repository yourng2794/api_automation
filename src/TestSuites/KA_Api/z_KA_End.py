...
from O2oUtility import WriteLog, WriteDebugLog, ReplaceJsonValue, ApiResponse, IniValue
...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Terminate] API POST KA end'
        self.CaseNote= ''
        self.TestCaseID= '28'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Portal] POST KA end API'+'\n')
        ...
        try:
            api_json = tooldir+'\\JsonFiles\\Api_Json\\ka_end.json'
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Portal] POST KA end API')
