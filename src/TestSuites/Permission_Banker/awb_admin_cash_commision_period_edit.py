...
from O2oUtility import AdminCashCommisionPeriodEdit
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST cash commision period edit'
        self.CaseNote= ''
        self.TestCaseID= '63'
...
#         ReplaceIniValue('admin_webPlayer_domain_create', 'API.bankerId', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST cash commision period edit'+'\n')        
        
        self.res_log, result = AdminCashCommisionPeriodEdit(self.res_log, 'Banker')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST cash commision period edit')
