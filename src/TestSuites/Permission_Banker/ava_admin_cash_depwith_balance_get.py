...
from O2oUtility import AdminCashDepwithBalanceGet
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST cash depwith balance get'
        self.CaseNote= ''
        self.TestCaseID= '63'
...
#         ReplaceIniValue('admin_webPlayer_domain_create', 'API.bankerId', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST cash depwith balance get'+'\n')        
        
        self.res_log, result = AdminCashDepwithBalanceGet(self.res_log, 'Banker')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST cash depwith balance get')
