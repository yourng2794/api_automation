...
from O2oUtility import AdminProviderPaymentEditInfoGet
from O2oUtility import AdminProviderPaymentEditCurrencyGet
from O2oUtility import AdminProviderPaymentEdit
...
class TestCase(BaseCase):
    ...
    def run(self, copy_result):
        ...
        payTypeName = IniValue('setting', 'API.payTypeName')
        ReplaceIniValue('admin_provider_payment_edit_info_get', 'API.paymentId', paymentId)
        self.res_log, result, pay_type_id, paymentUrl, paymentEnable = AdminProviderPaymentEditInfoGet(self.res_log, 'Banker', paymentName, paymentId, payTypeName)
        ...
        self.res_log, result, edit_currency = AdminProviderPaymentEditCurrencyGet(self.res_log, 'Banker')
        ...
        dict_json = {}
        dict_json['paymentId'] = paymentId
        dict_json['payname'] = paymentName
        dict_json['url'] = paymentUrl
        dict_json['currency'] = edit_currency
        dict_json['enable'] = paymentEnable
        with open(api_json, 'w') as outfile:
            json.dump(dict_json, outfile, indent=4, ensure_ascii=False)
        
        self.res_log, result = AdminProviderPaymentEdit(self.res_log, 'Banker')
