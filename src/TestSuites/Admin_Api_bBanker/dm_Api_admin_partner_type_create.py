...
from O2oUtility import WriteLog, WriteDebugLog, IniValue, ReplaceIniValue, AdminPartnerTypeCreate
...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST partner type create'
        self.CaseNote= ''
        self.TestCaseID= '250'
...
#         ReplaceIniValue('admin_partner_type_create', 'API.typeName', '')
#         ReplaceIniValue('admin_partner_type_create', 'API.permission', '')
#         ReplaceIniValue('admin_partner_type_create', 'API.currency', '')
#         ReplaceIniValue('admin_partner_type_create', 'API.payment', '')
#         ReplaceIniValue('admin_partner_type_create', 'API.provider', '')
#         ReplaceIniValue('admin_partner_type_create', 'API.stopbet', '')
#         ReplaceIniValue('admin_partner_type_create', 'API.handicap', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner type create'+'\n')
        
        number = IniValue('setting', 'API.number')
        typeName2 = IniValue('setting', 'API.typename2')+number
        ReplaceIniValue('admin_partner_type_create', 'API.typeName', typeName2)
        
        self.res_log, result = AdminPartnerTypeCreate(self.res_log, 'Banker')
        time.sleep(10)
        if(result == True):
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner type create')
