...
class TestCase(BaseCase):
    ...
    def run(self, copy_result):
        ...
        try:
            ...
            api_json = tooldir+'\\JsonFiles\\Api_Json\\admin_promo_edit.json'
            shutil.copyfile(tooldir+'\\JsonFiles\\Api_Json\\admin_promo_create.json', api_json)
            json_data = open(api_json, 'r+',encoding='UTF-8')
            edit_json = json.loads(json_data.read(), strict=False)
            edit_json['promoid'] = promoid
            edit_json['oldWebid'] = webid2
            edit_json['reward']['cash'] = '1'
            with open(api_json, 'w') as outfile:
                json.dump(edit_json, outfile, indent=4, ensure_ascii=False)
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_promo_edit', True, False)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_promo_edit... %s' % str(e))
