...
from O2oUtility import WriteLog, WriteDebugLog, ReplaceIniValue, DeleteFile, IniValue, AdminLogin
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Portal] API POST login Banker account (mobile)'
        self.CaseNote= ''
        self.TestCaseID= '376'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_login', 'API.code', '')
        ReplaceIniValue('admin_login', 'API.acc', '')
        ReplaceIniValue('admin_login', 'API.pwd', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Portal] API POST login Banker account (mobile)'+'\n')
        number = IniValue('setting', 'API.number')
        bankeracc = IniValue('setting', 'API.bankeracc')+number
        ReplaceIniValue('admin_login', 'API.acc', bankeracc)
        ReplaceIniValue('admin_login', 'API.pwd', bankeracc)
        
        self.res_log, result = AdminLogin(self.res_log, 'Banker', self.filename, 'mobile')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Portal] API POST login Banker account (mobile)')
