...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer risk web frame m'
        self.CaseNote= ''
        self.TestCaseID= '305'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer risk web frame m'+'\n')
        ...
        try:
            ...
            ReplaceIniValue('admin_webPlayer_risk_web_frame', 'API.ptid', '')
            ReplaceIniValue('admin_webPlayer_risk_web_frame', 'API.riskid', riskId)
            ReplaceIniValue('admin_webPlayer_risk_web_frame', 'API.api_path', 'api/webPlayer/risk/web/frame/m')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_risk_web_frame', False, False)
            ...
            if(return_value):
                ...
                for soup_vipLabelItem in soup_vipLabelItems:
                    if(str(soup_vipLabelItem.find('span').string).replace('\n','').replace(' ','') == webName2):
                        ...
                        if(soup_vipLabelItem.find('input').get('value') == webId2):
                            result_list2.append('PASS')
                            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get webId2: '+webId2)
                        else:
                            result_list2.append('FAIL')
                            self.res_log = WriteDebugLog(self.res_log, print_tab+'- FAIL get webId2: '+soup_vipLabelItem.find('input').get('value')+' != '+webId2)
                    else:
                        result_list2.append('FAIL')
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_risk_web_frame_m... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer risk web frame m')
