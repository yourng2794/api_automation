...
from O2oUtility import AdminVipEditGet
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer vip edit get'
        self.CaseNote= ''
        self.TestCaseID= '85'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_vip_edit_get', 'API.webId', '')
        ReplaceIniValue('admin_webPlayer_vip_edit_get', 'API.vipId', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer vip edit get'+'\n')
        ...
        ReplaceIniValue('admin_webPlayer_vip_edit_get', 'API.webId', webId2)
        ReplaceIniValue('admin_webPlayer_vip_edit_get', 'API.vipId', vipId2)
        deposit_times = 'None'
        deposit_total = '1000'
        
        self.res_log, result = AdminVipEditGet(self.res_log, 'Banker', webName2, vipId2, webDomainId2, deposit_times, deposit_total)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer vip edit get')
