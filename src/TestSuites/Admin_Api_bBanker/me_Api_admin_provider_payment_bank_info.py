...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider payment bank info'
        self.CaseNote= ''
        self.TestCaseID= '317'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payment bank info'+'\n')
        ...
        try:
            ...
            ReplaceIniValue('admin_provider_payment_bank_info', 'API.bankerId', bankerId)
            ...
            if(return_value):
                ...
                if(str(json_response['payname']) == paymentname):
                    result_list.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get payname: '+paymentname)
                else:
                    result_list.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get payname: '+str(json_response['payname'])+' != '+paymentname)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payment bank info')
