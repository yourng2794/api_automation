...
class TestCase(BaseCase):
    ...
    def run(self, copy_result):
        ...
        print_tab = '\n\t\t'
        try:
            playerAcc2 = IniValue('setting', 'API.playerAcc2')
            myName = IniValue('setting', 'API.myName')
            playerId2 = IniValue('setting', 'API.playerid2')
            ReplaceIniValue('admin_webPlayer_player_edit', 'API.playerPwd', playerAcc2)
            ReplaceIniValue('admin_webPlayer_player_edit', 'API.playerName', myName)
            ReplaceIniValue('admin_webPlayer_player_edit', 'API.playerId', playerId2)
        ...
        try:
            ...
            if(return_value):
                ...
                if(json_response['acc'] == playerAcc2):
                    result_list.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get playerAcc2: '+playerAcc2)
                else:
                    result_list.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get playerAcc2: '+json_response['acc']+' != '+playerAcc2)
