...
class TestCase(BaseCase):
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer web vip domain get'+'\n')
        ...
        print_tab = '\n\t\t'
        try:
            vipGroupId2 = IniValue('setting', 'API.vipGroupId2')
            webId2 = IniValue('setting', 'API.webId2')
            vipId2 = IniValue('setting', 'API.vipId2')
            webDomainId2 = IniValue('setting', 'API.webDomainId2')
            webDomain2 = IniValue('setting', 'API.webDomain2')
            vipName2 = IniValue('setting', 'API.vipName2')
            transferSet = IniValue('setting', 'API.transferSet')
            setTime = IniValue('setting', 'API.setTime')
            ReplaceIniValue('admin_webPlayer_web_vip_domain_get', 'API.vipGroupId', vipGroupId2)
            ReplaceIniValue('admin_webPlayer_web_vip_domain_get', 'API.webId', webId2)
            ReplaceIniValue('admin_webPlayer_web_vip_domain_get', 'API.vipId', vipId2)
            ...
            if(return_value):
                ...
                if(json_response['domainId'] == webDomainId2):
                    result_list2.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get webDomainId: '+webDomainId2)
                else:
                    result_list2.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get webDomainId: '+webDomainId2)
                    
                if(json_response['domainUrl'] == webDomain2):
                    result_list2.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get webDomain: '+webDomain2)
                else:
                    result_list2.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get webDomain: '+webDomain2)
                    
                if(json_response['vipName'] == vipName2):
                    result_list2.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get vipName: '+vipName2)
                else:
                    result_list2.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get vipName: '+vipName2)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer web vip domain get')
