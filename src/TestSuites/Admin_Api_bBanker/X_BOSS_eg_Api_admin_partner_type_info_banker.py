...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST partner type info banker'
        self.CaseNote= ''
        self.TestCaseID= '167'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner type info banker'+'\n')
        print_tab = '\n\t\t'
        try:
            typeId2 = IniValue('setting', 'API.typeid2')
            agentId2 = IniValue('setting', 'API.agentid2')
            ReplaceIniValue('admin_partner_type_info', 'API.ptTypeId', typeId2)
            ReplaceIniValue('admin_partner_type_info', 'API.ptParentId', agentId2)
            ...
            if(return_value):
                ...
                if(json_response.get('parent_acc') == agentAcc2):
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get agentAcc2: '+agentAcc2)
                    result_list2.append('PASS')
                else:
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get agentAcc2: '+json_response.get('parent_acc')+' != '+agentAcc2)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_partner_type_info_banker... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner type info banker')
