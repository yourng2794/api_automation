...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer web vip domain list'
        self.CaseNote= ''
        self.TestCaseID= '277'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer web vip domain list'+'\n')
        ...
        try:
            ...
            ReplaceIniValue('admin_webPlayer_web_vip_domain_list', 'API.webId', webId2)
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_web_vip_domain_list', False, False)
...
#                 {"2922f64a6ff9bcbf73fe114874689309":"10.10.10.220"}
                python_dict = {}
                python_dict[webDomainId2] = webDomain2
                if(response_data.replace('\n','').replace(' ','') == str(python_dict).replace("'",'"').replace(' ','')):
                    result_list2.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get response_data: '+str(python_dict).replace("'",'"').replace(' ',''))
                else:
                    result_list2.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get response_data: '+response_data.replace('\n','').replace(' ','')+' != '+str(python_dict).replace("'",'"').replace(' ',''))
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_web_vip_domain_list... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer web vip domain list')
