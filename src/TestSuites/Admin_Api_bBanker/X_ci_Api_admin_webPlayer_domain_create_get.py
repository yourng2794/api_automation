...
from O2oUtility import WriteLog, WriteDebugLog, IniValue, ReplaceIniValue, AdminDomainCreateGet
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer domain create get'
        self.CaseNote= ''
        self.TestCaseID= '236'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_domain_create_get', 'API.bankerId', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer domain create get'+'\n')
        
        # Part1: Replace bankerId by partnerId setting.ini 
        number = IniValue('setting', 'API.number')
        bankerId = IniValue('setting', 'API.bankerId')
        bankerAcc = IniValue('setting', 'API.bankerAcc')+number
        ReplaceIniValue('admin_webPlayer_domain_create_get', 'API.bankerId', bankerId)
        bankerId = IniValue('admin_webPlayer_domain_create_get', 'API.bankerid')
        
        apiResponse = IniValue('admin_webPlayer_domain_create_get', 'API.api_response')
        python_dict = json.loads(apiResponse)
        python_dict['bankerId'] = bankerId
        python_dict['bankerAcc'] = bankerAcc
        
        ReplaceIniValue('admin_webPlayer_domain_create_get', 'API.api_response', str(python_dict).replace("'",'"').replace(' ',''))
        
        self.res_log, result = AdminDomainCreateGet(self.res_log, 'Banker')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer domain create get')
