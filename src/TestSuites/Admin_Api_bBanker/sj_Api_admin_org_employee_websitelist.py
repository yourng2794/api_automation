...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST org. employee websitelist'
        self.CaseNote= ''
        self.TestCaseID= '371'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. employee websitelist'+'\n')
        print_tab = '\n\t\t'
        try:
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_org_employee_websitelist', False, False)
            ...
            if(return_value):
                ...
                for element in json_response:
                    if(element['web_id'] == webId2 and element['web_name'] == webName2):
                        self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get (webName2, webId2): ('+webName2+', '+webId2+')')
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_org_employee_websitelist... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. employee websitelist')
