...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider payment list frame'
        self.CaseNote= ''
        self.TestCaseID= '313'
    ...
    def run(self, copy_result):
        ...
        result_list2 = []
        paymentid_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payment list frame'+'\n')
        ...
        try:
            ...
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_provider_payment_list_frame', False, False)
            ...
            if(return_value):
                ...
                if(len(soup_lis) != 0):
                    ...
                    for soup_li in soup_lis:
                        if(str(soup_li.find('div', class_='status-tag').string).replace('\n','').replace(' ','') == '启用'):
                            result_list2.append('PASS')
                            paymentid_list.append(soup_li.find('a', attrs={'href':'provider_cash_data.html'}).get('id'))
                            paymentid_list.append(str(soup_li.find('div', class_='box-15').string).replace('\n','').replace(' ',''))
                        else:
                            result_list2.append('FAIL')
                    list_index = random.randrange(0,len(paymentid_list),2)
                    paymentId = paymentid_list[list_index]
                    paymentName = paymentid_list[list_index+1]
                    ReplaceIniValue('setting', 'API.paymentid', paymentId)
                    ReplaceIniValue('setting', 'API.paymentname', paymentName)
                else:
                    result_list.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+"- There's no paymentId")
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_provider_payment_list_frame... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payment list frame') 
