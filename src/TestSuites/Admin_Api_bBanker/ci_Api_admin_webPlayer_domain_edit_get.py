...
from O2oUtility import WriteLog, WriteDebugLog, IniValue, ReplaceIniValue, AdminDomainEditGet
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST web domain edit get'
        self.CaseNote= ''
        self.TestCaseID= '238'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_domain_edit_get', 'API.webDomainId', '')
        ReplaceIniValue('admin_webPlayer_domain_edit_get', 'API.bankerId', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST web domain edit get'+'\n')
        ...
        ReplaceIniValue('admin_webPlayer_domain_edit_get', 'API.webDomainId', webDomainId2)
        ReplaceIniValue('admin_webPlayer_domain_edit_get', 'API.bankerId', bankerId)
        
        self.res_log, result = AdminDomainEditGet(self.res_log, 'Banker', bankerAcc)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST web domain edit get')
