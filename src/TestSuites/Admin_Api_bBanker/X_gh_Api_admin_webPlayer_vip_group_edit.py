...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer vip group edit'
        self.CaseNote= ''
        self.TestCaseID= '275'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer vip group edit'+'\n')
        ...
        try:
            ...
            ReplaceIniValue('admin_webPlayer_vip_group_edit', 'API.vipGroupId', vipGroupId2)
            ReplaceIniValue('admin_webPlayer_vip_group_edit', 'API.vipid[]', vipId2)
            ReplaceIniValue('admin_webPlayer_vip_group_edit', 'API.vipGroupName', 'new'+vipGroupName2)
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_vip_group_edit', True, False)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_vip_group_edit... %s' % str(e))
        
        finally:
            self.res_log = WriteDebugLog(self.res_log, print_tab+'---')
        
        try:
            ReplaceIniValue('admin_webPlayer_vip_group_edit', 'API.vipGroupName', vipGroupName2)
            return_value2, response_time2, response_data2, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_vip_group_edit', True, False)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer vip group edit')
