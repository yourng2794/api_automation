...
class TestCase(BaseCase):
    ...
    def run(self, copy_result):
        ...
        try:
            ...
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_provider_bankCard_list_frame', False, False)
                ...
                for soup_li in soup_lis:
                    ...
                    for soup_div in soup_divs:
                        if(str(soup_div.string).replace('\n','').replace(' ','') == bankName2):
                            ...
                            cardId2 = soup_li.find("a", attrs={"href":"provider_card_edit.html"}).get("id")
                            ReplaceIniValue('setting', 'API.cardid2', cardId2)
                            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get cardId2: '+cardId2)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_provider_bankCard_list_frame... %s' % str(e))
