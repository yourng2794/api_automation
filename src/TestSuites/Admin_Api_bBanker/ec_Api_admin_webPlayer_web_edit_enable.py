...
from O2oUtility import WriteLog, WriteDebugLog, IniValue, ReplaceIniValue, AdminWebEdit
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST admin webPlayer web edit enable'
        self.CaseNote= ''
        self.TestCaseID= '248'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_web_edit', 'API.bankerId', '')
        ReplaceIniValue('admin_webPlayer_web_edit', 'API.use', '')
        ReplaceIniValue('admin_webPlayer_web_edit', 'API.webdomainid', '')
        ReplaceIniValue('admin_webPlayer_web_edit', 'API.webName', '')
        ReplaceIniValue('admin_webPlayer_web_edit', 'API.webId', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST admin webPlayer web edit enable'+'\n')
        ...
        webname = IniValue('setting', 'API.webname2')+number
        ReplaceIniValue('admin_webPlayer_web_edit', 'API.bankerId', bankerId)
        ReplaceIniValue('admin_webPlayer_web_edit', 'API.use', '3')
        python_dict = {}
        python_dict[webDomainId2] = '1'
        ReplaceIniValue('admin_webPlayer_web_edit', 'API.webdomainid', str(python_dict).replace("'",'"').replace(' ',''))
        ReplaceIniValue('admin_webPlayer_web_edit', 'API.webName', webname)
        ReplaceIniValue('admin_webPlayer_web_edit', 'API.webId', webId2)
        self.res_log, result = AdminWebEdit(self.res_log, 'Banker')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST admin webPlayer web edit enable')
