...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider payOption edit get withdraw'
        self.CaseNote= ''
        self.TestCaseID= '333'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption edit get withdraw'+'\n')
        ...
        try:
            ...
            ReplaceIniValue('admin_provider_payOption_edit_get_withdraw', 'API.payOptionId', payOptionId2)
            ReplaceIniValue('admin_provider_payOption_edit_get_withdraw', 'API.currency', currency)
            ReplaceIniValue('admin_provider_payOption_edit_get_withdraw', 'API.payOption', '1')
            
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_provider_payOption_edit_get_withdraw', False, False)
            ...
            if(return_value):
                ...
                list_payment = []
                list_payment.append(paymentId)
                list_payment.append('1')
                json_response = json.loads(response_data)
                if(json_response[paymentName] == list_payment):
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get paymentName2: '+str(list_payment))
                    result_list.append('PASS')
                else:
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get paymentName2: '+str(json_response[paymentName])+' != '+str(list_payment))
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_provider_payOption_edit_get_withdraw... %s' % str(e))
            
        finally:
            self.res_log = WriteDebugLog(self.res_log, print_tab+'---')
        
        try:
            ReplaceIniValue('admin_provider_payOption_edit_get_withdraw', 'API.payOption', '2')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_provider_payOption_edit_get_withdraw', False, False)
            WriteLog(print_tab+'Response data: '+ response_data)
            if(return_value):
                self.res_log = WriteDebugLog(self.res_log, print_tab+'- API success (status and response are correct).')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption edit get withdraw')
