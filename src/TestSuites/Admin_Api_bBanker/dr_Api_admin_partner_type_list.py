...
from O2oUtility import WriteLog, WriteDebugLog, IniValue, AdminPartnerTypeList
...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST partner type list'
        self.CaseNote= ''
        self.TestCaseID= '228'
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner type list'+'\n')
        
        number = IniValue('setting', 'API.number')
        typeName2 = IniValue('setting', 'API.typename2')+number
        typeId2 = IniValue('setting', 'API.typeid2')
        self.res_log, result = AdminPartnerTypeList(self.res_log, 'Banker', typeName2, typeId2)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner type list')
