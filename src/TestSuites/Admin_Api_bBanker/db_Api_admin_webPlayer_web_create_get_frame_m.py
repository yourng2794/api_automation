...
from O2oUtility import WriteLog, WriteDebugLog, IniValue, ReplaceIniValue, AdminWebCreateGetFrame
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST admin webPlayer web create get frame m'
        self.CaseNote= ''
        self.TestCaseID= '242'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_web_create_get_frame', 'API.bankerId', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST admin webPlayer web create get frame m'+'\n')
        
        number = IniValue('setting', 'API.number')
        str_webDomain = IniValue('setting', 'API.webdomain2')
        list_webDomain = []
        for each_webDomain in str_webDomain.split(', '):
            list_webDomain.append(each_webDomain+number)
        webDomain2 = ', '.join(list_webDomain)
        bankerId = IniValue('setting', 'API.bankerId')
        ReplaceIniValue('admin_webPlayer_web_create_get_frame', 'API.bankerId', bankerId)
        
        self.res_log, result = AdminWebCreateGetFrame(self.res_log, 'Banker', webDomain2, web_or_mobile = 'Mobile')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST admin webPlayer web create get frame m')
