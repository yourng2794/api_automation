...
from O2oUtility import WriteLog, WriteDebugLog, IniValue, ReplaceIniValue, AdminPartnerInfoGet
...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST partner info get'
        self.CaseNote= ''
        self.TestCaseID= '379'
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner info get'+'\n')
        
        webMasterId2 = IniValue('setting', 'API.webMasterId2')
        webMaster_percent = IniValue('setting', 'API.webMaster_percent')
        ReplaceIniValue('admin_partner_info_get', 'API.ptId', webMasterId2)
        number = IniValue('setting', 'API.number')
        webMasterAcc2 = IniValue('setting', 'API.webMasterAcc2')+number
        webId2 = ''
        
        self.res_log, result = AdminPartnerInfoGet(self.res_log, 'Banker', webMasterId2, webMasterAcc2, webId2, webMaster_percent)
...
#         try:
#             agentId2 = IniValue('setting', 'API.agentid2')
#             ReplaceIniValue('admin_partner_info_get', 'API.ptId', agentId2)
#             return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_partner_info_get', False, False)
...
#                 agentAcc2 = IniValue('setting', 'API.agentacc2')
#                 webId2 = IniValue('setting', 'API.webid2')
#                 if(json_response.get('pt_acc') == agentAcc2):
#                     self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get pt_acc: '+agentAcc2)
#                     result_list2.append('PASS')
#                 else:
#                     self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get pt_acc: '+json_response.get('pt_acc')+' != '+agentAcc2)
#                     result_list2.append('FAIL')
#                 
#                 if(json_response.get('pt_id') == agentId2):
#                     self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get pt_id: '+agentId2)
#                     result_list2.append('PASS')
#                 else:
#                     self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get pt_id: '+json_response.get('pt_id')+' != '+agentId2)
#                     result_list2.append('FAIL')
#                 
#                 if(json_response.get('pt_website')[0].get('web_id') == webId2):
#                     self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get webId2: '+webId2)
#                     result_list2.append('PASS')
#                 else:
#                     self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get webId2: '+json_response.get('banker_website')[0].get('web_id')+' != '+webId2)
#                     result_list2.append('FAIL')
...
#         except Exception as e:
#             result_list.append('FAIL')
#             self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_partner_info_get... %s' % str(e))
#         
#         finally:
#             self.res_log = WriteDebugLog(self.res_log, print_tab+'---')
#         
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner info get')
