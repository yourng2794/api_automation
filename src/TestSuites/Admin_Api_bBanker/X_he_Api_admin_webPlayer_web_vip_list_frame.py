...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer web vip list frame'
        self.CaseNote= ''
        self.TestCaseID= '280'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer web vip list frame'+'\n')
        ...
        try:
            ...
            vipGroupName2 = IniValue('setting', 'API.vipGroupName2')
            transferSet = IniValue('setting', 'API.transferSet')
            setTime = IniValue('setting', 'API.setTime')
            ReplaceIniValue('admin_webPlayer_web_vip_list_frame', 'API.webId', webId2)
            ReplaceIniValue('admin_webPlayer_web_vip_list_frame', 'API.api_path', 'api/webPlayer/web/vip/list/frame')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_web_vip_list_frame', False, False)
            ...
            if(return_value):
                ...
                soup_selecteds = soup.find_all('option')
#                 soup_selecteds = soup.find_all("option", selected='selected')
#                 soup_selecteds = soup.find_all('option', attrs={"selected": ""})
                soup_selecteds = soup.find_all("option", selected=True)
                vipGroupName_2 = str(soup_selecteds[0].string).replace('\n','').replace(' ','')
                if(vipGroupName_2 == vipGroupName2):
                    result_list2.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get vipGroupName2: '+vipGroupName2)
                else:
                    result_list2.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get vipGroupName2: '+vipGroupName_2+' != '+vipGroupName2)
                ...
                if(soup_selecteds[1].get('value') == transferSet):
                    ...
                    result_list2.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get transferSet: '+soup_selecteds[1].get('value')+' != '+transferSet)
                ...
                if(str(soup_selecteds[2].string).replace('\n','').replace(' ','') == setTime):
                    ...
                    result_list2.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get setTime: '+str(soup_selecteds[2].string).replace('\n','').replace(' ','')+' != '+setTime)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_web_vip_list_frame... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer web vip list frame')
