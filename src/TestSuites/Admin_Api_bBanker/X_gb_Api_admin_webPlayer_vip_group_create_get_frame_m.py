...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer vip group create get frame m'
        self.CaseNote= ''
        self.TestCaseID= '269'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer vip group create get frame m'+'\n')
        ...
        try:
            ...
            ReplaceIniValue('admin_webPlayer_vip_group_create_get_frame', 'API.bankerId', partnerid)
            ReplaceIniValue('admin_webPlayer_vip_group_create_get_frame', 'API.api_path', 'api/webPlayer/vip/group/create/get/frame/m')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_vip_group_create_get_frame', False, False)
            ...
            if(return_value):
                ...
                vip_Name = soup.find("div", id=vipId2).string.replace('\n','').replace(' ','')
                if(vip_Name == vipName2):
                    ...
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get vipName2: '+vip_Name+' != '+vipName2)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_vip_group_create_get_frame_m... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer vip group create get frame m')
