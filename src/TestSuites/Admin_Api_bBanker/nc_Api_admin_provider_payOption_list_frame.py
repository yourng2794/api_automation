...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider payOption list frame'
        self.CaseNote= ''
        self.TestCaseID= '323'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption list frame'+'\n')
        ...
        try:
            ...
            ReplaceIniValue('admin_provider_payOption_list_frame', 'API.bankerId', bankerId)
            ReplaceIniValue('admin_provider_payOption_list_frame', 'API.api_path', 'api/provider/payOption/list/frame')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_provider_payOption_list_frame', False, False)
                ...
                for soup_li in soup_lis:
                    ...
                    for soup_payOptionId in soup_payOptionIds:
                        if(str(soup_payOptionId.string).replace('\n','').replace(' ','') == payOptionName2):
                            ...
                            payOptionId2 = soup_li.find("a", attrs={'href':'provider_pay_edit.html'}).get("id")
                            ReplaceIniValue('setting', 'API.payOptionId2', payOptionId2)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_provider_payOption_list_frame... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption list frame')
