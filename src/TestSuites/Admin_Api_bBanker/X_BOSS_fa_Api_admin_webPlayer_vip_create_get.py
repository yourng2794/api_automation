...
class TestCase(BaseCase):
    ...
    def run(self, copy_result):
        ...
        try:
            ...
            ReplaceIniValue('admin_webPlayer_vip_create_get', 'API.bankerId', partnerid)
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_vip_create_get', False, False)
            ...
            if(return_value):
                ...
                if('{"'+partnerid+'":"'+agentAcc2+'"}' in response_data):
                    result_list2.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get agentAcc2: {"'+partnerid+'":"'+agentAcc2+'"}')
                else:
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_vip_create_get... %s' % str(e))
