...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer risk edit'
        self.CaseNote= ''
        self.TestCaseID= '303'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer risk edit'+'\n')
        ...
        try:
            ...
            shutil.copyfile(tooldir+'\\JsonFiles\\Api_Json\\admin_risk_create.json', api_json)
            json_data = open(api_json, 'r+',encoding='UTF-8')
            edit_json = json.loads(json_data.read(), strict=False)
            edit_json['riskid'] = riskid
            edit_json['oldWebid'] = webId2
            with open(api_json, 'w') as outfile:  
                json.dump(edit_json, outfile, indent=4, ensure_ascii=False)
            
            dict_opt = {}
            dict_opt['type'] = 'ipLogin'
            dict_opt['interval'] = '1'
            dict_opt['time'] = 'min'
            dict_opt['amount'] = ''
            dict_opt['count'] = '1'
            
            ReplaceJsonValue(api_json, 'opt|0', [dict_opt])
            
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_risk_edit', True, False)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_risk_edit... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer risk edit')
