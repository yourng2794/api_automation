...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider bankCard edit get'
        self.CaseNote= ''
        self.TestCaseID= '311'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider bankCard edit get'+'\n')
        ...
        print_tab = '\n\t\t'
        try:
            cardId2 = IniValue('setting', 'API.cardid2')
            ReplaceIniValue('admin_provider_bankCard_edit_get', 'API.cardId', cardId2)
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_provider_bankCard_edit_get', False, False)
            ...
            if(return_value):
                ...
                amount = IniValue('admin_provider_bankCard_create', 'API.amount')
                maxdeposit = IniValue('admin_provider_bankCard_create', 'API.maxdeposit')
                maxwithdraw = IniValue('admin_provider_bankCard_create', 'API.maxwithdraw')
                mindeposit = IniValue('admin_provider_bankCard_create', 'API.mindeposit')
                bankname2 = IniValue('setting', 'API.bankname2')
                branchname2 = IniValue('setting', 'API.branchname2')
                bankacc2 = IniValue('setting', 'API.bankacc2')
                accname2 = IniValue('setting', 'API.accname2')
                json_response = json.loads(response_data)
                if(json_response['acc_name'] == accname2):
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get accname2: '+accname2)
                    result_list2.append('PASS')
                else:
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get accname2: '+json_response['acc_name'] +' != '+accname2)
                    result_list2.append('FAIL')
                
                if(json_response['bank_acc'] == bankacc2):
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get bankacc2: '+bankacc2)
                    result_list2.append('PASS')
                else:
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get bankacc2: '+json_response['bank_acc'] +' != '+bankacc2)
                    result_list2.append('FAIL')
                
                if(json_response['bank_name'] == bankname2):
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get bankname2: '+bankname2)
                    result_list2.append('PASS')
                else:
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get bankname2: '+json_response['bank_name'] +' != '+bankname2)
                    result_list2.append('FAIL')
                
                if(json_response['branch_name'] == branchname2):
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get branchname2: '+branchname2)
                    result_list2.append('PASS')
                else:
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get branchname2: '+json_response['branch_name'] +' != '+branchname2)
                    result_list2.append('FAIL')
                
                if(str(json_response['enable']) == '1'):
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get enable: 1')
                    result_list2.append('PASS')
                else:
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get enable: '+json_response['enable'] +' != 1')
                    result_list2.append('FAIL')
                
                if(str(json_response['amount']) == amount):
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get amount: '+amount)
                    result_list2.append('PASS')
                else:
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get amount: '+json_response['amount'] +' != '+amount)
                    result_list2.append('FAIL')
                
                if(str(json_response['maxdeposit']) == maxdeposit):
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get maxdeposit: '+maxdeposit)
                    result_list2.append('PASS')
                else:
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get maxdeposit: '+json_response['maxdeposit'] +' != '+maxdeposit)
                    result_list2.append('FAIL')
                
                if(str(json_response['maxwithdraw']) == maxwithdraw):
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get maxwithdraw: '+maxwithdraw)
                    result_list2.append('PASS')
                else:
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get maxwithdraw: '+json_response['maxwithdraw'] +' != '+maxwithdraw)
                    result_list2.append('FAIL')
                
                if(str(json_response['mindeposit']) == mindeposit):
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get mindeposit: '+mindeposit)
                    result_list2.append('PASS')
                else:
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get mindeposit: '+json_response['mindeposit'] +' != '+mindeposit)
                    result_list2.append('FAIL')
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_provider_bankCard_edit_get... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider bankCard edit get')
