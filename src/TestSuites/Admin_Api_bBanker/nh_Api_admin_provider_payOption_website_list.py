...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider payOption website list'
        self.CaseNote= ''
        self.TestCaseID= '328'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption website list'+'\n')
        ...
        try:
            ...
            ReplaceIniValue('admin_provider_payOption_website_list', 'API.payOptionId', payOptionId2)
            ReplaceIniValue('admin_provider_payOption_website_list', 'API.bankerId', '')
            ReplaceIniValue('admin_provider_payOption_website_list', 'API.api_path', 'api/provider/payOption/website/list')
            ...
            if(return_value):
                ...
                json_response2 = json.loads(json_response.get('list'))
                if(json_response['payoption_name'] == payOptionName2):
                    ...
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get payOptionName2: '+json_response['payoption_name']+ ' != '+ payOptionName2)
                    result_list.append('FAIL')
                    
                if(json_response2.get(webId2) == webName2):
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get webName2: '+webName2)
                    result_list.append('PASS')
                else:
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get webName2: '+json_response2.get(webId2)+ ' != '+ webName2)
                    result_list.append('FAIL')
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_provider_payOption_website_list... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption website list')
