...
from O2oUtility import WriteLog, WriteDebugLog, IniValue, ReplaceIniValue, AdminWebEditGetFrame
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer web edit get frame m'
        self.CaseNote= ''
        self.TestCaseID= '246'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_web_edit_get_frame', 'API.webId', '')
        ReplaceIniValue('admin_webPlayer_web_edit_get_frame', 'API.bankerId', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer web edit get frame m'+'\n')
        ...
        ReplaceIniValue('admin_webPlayer_web_edit_get_frame', 'API.webId', webId2)
        ReplaceIniValue('admin_webPlayer_web_edit_get_frame', 'API.bankerId', bankerId)
        
        self.res_log, result = AdminWebEditGetFrame(self.res_log, 'Banker', webDomain2, webDomainId2, web_or_mobile = 'Mobile')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer web edit get frame m')
