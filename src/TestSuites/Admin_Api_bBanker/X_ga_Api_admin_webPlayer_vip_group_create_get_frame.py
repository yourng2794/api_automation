...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer vip group create get frame'
        self.CaseNote= ''
        self.TestCaseID= '268'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer vip group create get frame'+'\n')
        ...
        try:
            ...
            if(return_value):
                ...
                if(soup.find("div", id=vipId2).string.replace('\n','').replace(' ','') == vipName2):
                    ...
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get vipName2: '+soup.find("div", id=vipId2).string.replace('\n','').replace(' ','')+' != '+vipName2)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_vip_group_create_get_frame... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer vip group create get frame')
