...
from O2oUtility import AdminPartnerTypeGet
...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST partner type get'
        self.CaseNote= ''
        self.TestCaseID= '253'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_partner_type_get', 'API.typeId', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner type get'+'\n')
        
        number = IniValue('setting', 'API.number')
        typeId2 = IniValue('setting', 'API.typeid2')
        ReplaceIniValue('admin_partner_type_get', 'API.typeId', typeId2)
        typeName2 = IniValue('setting', 'API.typename2')+number
        
        self.res_log, result = AdminPartnerTypeGet(self.res_log, 'Banker', typeName2, 'Web')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner type get')
