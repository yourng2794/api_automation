...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer vip group edit get frame m'
        self.CaseNote= ''
        self.TestCaseID= '274'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer vip group edit get frame m'+'\n')
        ...
        try:
            ...
            ReplaceIniValue('admin_webPlayer_vip_group_edit_get_frame', 'API.vipGroupId', vipGroupId2)
            ReplaceIniValue('admin_webPlayer_vip_group_edit_get_frame', 'API.api_path', 'api/webPlayer/vip/group/edit/get/frame/m')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_vip_group_edit_get_frame', False, False)
            ...
            if(return_value):
                ...
                vipName_2 = str(soup.find("div", id=vipId2).string).replace('\n','').replace(' ','')
                if(vipName_2 == vipName2):
                    result_list.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get vipName2: '+vipName2)
                else:
                    result_list.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get vipName2: '+vipName_2+' != '+vipName2)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_vip_group_edit_get_frame_m... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer vip group edit get frame m')
