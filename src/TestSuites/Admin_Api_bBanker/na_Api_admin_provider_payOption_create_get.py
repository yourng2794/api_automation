...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider payOption create get'
        self.CaseNote= ''
        self.TestCaseID= '384'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption create get'+'\n')
        ...
        try:
            ...
            ReplaceIniValue('admin_provider_payOption_create_get', 'API.bankerId', bankerId)
            
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_provider_payOption_create_get', False, False)
            ...
            if(return_value):
                ...
                pt_acc = IniValue('setting', 'API.acc')
                json_response = json.loads(response_data)
                if(json_response['pt_acc'] == pt_acc):
                    result_list.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get pt_acc: '+pt_acc)
                else:
                    result_list.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get pt_acc: '+json_response['pt_acc']+' != '+pt_acc)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_provider_payOption_create_get... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption create get')
