...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider payment list frame m'
        self.CaseNote= ''
        self.TestCaseID= '314'
    ...
    def run(self, copy_result):
        ...
        result_list2 = []
        paymentid_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payment list frame m'+'\n')
        ...
        try:
            ...
            if(return_value):
                ...
                if(len(soup_lis) != 0):
                    ...
                    for soup_li in soup_lis:
                        if(str(soup_li.find('span', class_='status-type').string).replace('\n','').replace(' ','') == '启用'):
                            result_list2.append('PASS')
                            paymentid_list.append(soup_li.find('div', class_='crm-edit').get('id'))
                            paymentid_list.append(str(soup_li.find('div', class_='crm-name').string).replace('\n','').replace(' ',''))
                        else:
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_provider_payment_list_frame_m... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payment list frame m') 
