...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider payment bank link edit'
        self.CaseNote= ''
        self.TestCaseID= '140'
    ...
    def run(self, copy_result):
        ...
        result_list = []
#         list_banknames = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payment bank link edit'+'\n')
        ...
        print_tab = '\n\t\t'
        # Part 1: Get payment bank status
        try:
            partnerId = IniValue('setting', 'API.partnerid')
            paymentId = IniValue('setting', 'API.paymentid')
            currency = IniValue('setting', 'API.currency')
            ReplaceIniValue('admin_provider_payment_bank_get', 'API.paymentId', paymentId)
            ReplaceIniValue('admin_provider_payment_bank_get', 'API.bankerId', '')
            ReplaceIniValue('admin_provider_payment_bank_get', 'API.currency', currency)
            ReplaceIniValue('admin_provider_payment_bank_get', 'API.option', '1')
            
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_provider_payment_bank_get', False, False)
            ...
            if(return_value):
                ...
                if(str(json_response[bankName2]) == '0'):
                    result_list.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get (bankName2, value): '+bankName2+', 0')
                else:
                    result_list.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get (bankName2, value)')
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_provider_payment_bank_get... %s' % str(e))
            
        finally:
            self.res_log = WriteDebugLog(self.res_log, print_tab+'---')
        
        # Part 2: Set payment bank.
        try:
...
#             json_dict['withdraw'][currency]['0'] = list_banknames[0]
        ...
        # Part 3: Check payment bank status again.
        try:
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_provider_payment_bank_get', False, False)
            WriteLog(print_tab+'Response data: '+ response_data)
            if(return_value):
                ...
                bankName2 = IniValue('setting', 'API.bankname2')
                json_response = json.loads(response_data)
                if(str(json_response[bankName2]) == '1'):
                    result_list.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get (bankName, value): '+bankName2+', 1')
                else:
                    result_list.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get (bankName, value)')
                self.res_log = WriteDebugLog(self.res_log, print_tab+'---')
        ...
        # Part 4: Set payment bank link back to origin.
        try:
            ReplaceJsonValue(api_json, 'withdraw|'+currency, '{}')
            with open(api_json, 'w') as outfile:
                json.dump(json_dict, outfile, indent=4, ensure_ascii=False)
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_provider_payment_bank_link_edit', True, False)
            WriteLog(print_tab+'Response data: '+ response_data)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payment bank link edit')
