...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider payOption list frame m'
        self.CaseNote= ''
        self.TestCaseID= '324'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption list frame m'+'\n')
        ...
        try:
                ...
                for soup_li in soup_lis:
                    ...
                    for soup_payOptionId in soup_payOptionIds:
                        if(str(soup_payOptionId.string).replace('\n','').replace(' ','') == payOptionName2):
                            ...
                            payOptionId2 = soup_li.find("a", attrs={'alt':'修改'}).get("id")
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_provider_payOption_list_frame_m... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption list frame m')
