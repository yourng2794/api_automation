...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer vip group list frame m'
        self.CaseNote= ''
        self.TestCaseID= '272'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer vip group list frame m'+'\n')
        ...
        try:
            ...
            ReplaceIniValue('admin_webPlayer_vip_group_list_frame', 'API.bankerId', partnerid)
            ReplaceIniValue('admin_webPlayer_vip_group_list_frame', 'API.api_path', 'api/webPlayer/vip/group/list/frame/m')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_vip_group_list_frame', False, False)
            ...
            if(return_value):
                ...
                if(vipGroupName2 in response_data):
                    soup = BeautifulSoup(response_data, 'html.parser')
                    soup_vipGroupIds = soup.find_all("div")
                    for soup_vipGroupId in soup_vipGroupIds:
                        if(str(soup_vipGroupId.string).replace('\n','').replace(' ','') == vipGroupName2):
                            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get vipGroupName2: '+vipGroupName2)
                            vipGroupId2 = soup_vipGroupId.get('id')
                            ReplaceIniValue('setting', 'API.vipGroupId2', vipGroupId2)
                            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get vipGroupId2: '+vipGroupId2)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_vip_group_list_frame_m... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer vip group list frame m')
