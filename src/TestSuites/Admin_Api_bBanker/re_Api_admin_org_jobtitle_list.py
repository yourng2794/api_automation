...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST org. jobtitle list'
        self.CaseNote= ''
        self.TestCaseID= '359'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. jobtitle list'+'\n')
        print_tab = '\n\t\t'
        try:
            depId2 = IniValue('setting', 'API.depid2')
            depTitle2 = IniValue('setting', 'API.deptitle2')
            depTitleId2 = IniValue('setting', 'API.deptitleid2')
            ReplaceIniValue('admin_org_jobtitle_list', 'API.depId', depId2)
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_org_jobtitle_list', False, False)
            ...
            if(return_value):
                ...
                for each_element in list(json_list):
                    if(each_element['dp_name'] == depTitle2):
                        result_list2.append('PASS')
                        self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get depTitle2: '+depTitle2)
                        if(each_element['dp_sid'] == depTitleId2):
                            result_list2.append('PASS')
                            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get depTitleId2: '+depTitleId2)
                        else:
                            result_list2.append('FAIL')
                            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get depTitleId2: '+each_element['dp_sid']+' != '+depTitleId2)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_org_jobtitle_list... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. jobtitle list')
