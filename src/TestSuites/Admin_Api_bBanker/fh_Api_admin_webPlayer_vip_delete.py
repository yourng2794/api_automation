...
class TestCase(BaseCase):
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_vip_delete', 'API.webId', '')
    ...
    def run(self, copy_result):
        ...
        webDomainId2 = IniValue('setting', 'API.webDomainId2')
        str_webDomain = IniValue('setting', 'API.webDomain2')
        webDomain2 = str_webDomain.split(', ')[0]+number
        webId2 = IniValue('setting', 'API.webId2')
        ReplaceIniValue('admin_webPlayer_vip_create', 'API.webId', webId2)
        ReplaceIniValue('admin_webPlayer_vip_create', 'API.bankerId', bankerId)
        ReplaceIniValue('admin_webPlayer_vip_create', 'API.vipname', vipName2)
        ReplaceIniValue('admin_webPlayer_vip_create', 'API.depositTimes', '100')
        ReplaceIniValue('admin_webPlayer_vip_create', 'API.depositTotal', '')
        ReplaceIniValue('admin_webPlayer_vip_create', 'API.webDomainId', webDomainId2)
        ReplaceIniValue('admin_webPlayer_vip_create', 'API.webDomainUrl', webDomain2)
        
        self.res_log, result = AdminVipCreate(self.res_log, 'Banker')
        result_list.append(result)
        
        # Part 2: Get VIP ID from web.
        ReplaceIniValue('admin_webPlayer_vip_list_frame', 'API.webId', webId2)
        ReplaceIniValue('admin_webPlayer_vip_list_frame', 'API.bankerId', bankerId)
        
        self.res_log, result, vipId2 = AdminVipListFrame(self.res_log, 'Banker', vipName2, 'Web')
        result_list.append(result)
        
        # Part 3: Get VIP ID from mobile web.
        
        # Part 4: Delete VIP.
        ReplaceIniValue('admin_webPlayer_vip_delete', 'API.webId', webId2)
        ReplaceIniValue('admin_webPlayer_vip_delete', 'API.vipId', vipId2)
        self.res_log, result = AdminVipDelete(self.res_log, 'Banker')
        result_list.append(result)
