...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider payment bank get'
        self.CaseNote= ''
        self.TestCaseID= '318'
    ...
    def run(self, copy_result):
        ...
        result_list2 = []
        result_list3 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payment bank get'+'\n')
        ...
        try:
            ...
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_provider_bankCard_edit', True, False)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_provider_bankCard_create_edit... %s' % str(e))
        ...
        try:
            ...
            ReplaceIniValue('admin_provider_payment_bank_get', 'API.bankerId', bankerId)
            ReplaceIniValue('admin_provider_payment_bank_get', 'API.currency', 'CNY')
        ...
        try:
            ...
            ReplaceIniValue('admin_provider_payment_bank_get', 'API.bankerId', bankerId)
            ReplaceIniValue('admin_provider_payment_bank_get', 'API.currency', 'CNY')
            ReplaceIniValue('admin_provider_payment_bank_get', 'API.option', '2')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payment bank get')
