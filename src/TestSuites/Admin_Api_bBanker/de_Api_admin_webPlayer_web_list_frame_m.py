...
from O2oUtility import WriteLog, WriteDebugLog, IniValue, ReplaceIniValue, AdminWebListFrame
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer web list frame m'
        self.CaseNote= ''
        self.TestCaseID= '244'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_web_list_frame', 'API.bankerId', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer web list frame m'+'\n')
        
        number = IniValue('setting', 'API.number')
        webName2 = IniValue('setting', 'API.webname2')+number
        bankerId = IniValue('setting', 'API.bankerId')
        ReplaceIniValue('admin_webPlayer_web_list_frame', 'API.bankerId', bankerId)
        
        self.res_log, result, webId2 = AdminWebListFrame(self.res_log, 'Banker', webName2, 'Mobile')
        
        if(result == True):
            ReplaceIniValue('setting', 'API.webid2', webId2)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer web list frame m')
