...
from O2oUtility import WriteLog, WriteDebugLog, IniValue, ReplaceIniValue, AdminDomainCreate
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer domain create'
        self.CaseNote= ''
        self.TestCaseID= '233'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_domain_create', 'API.webDomain[]', '')
        ReplaceIniValue('admin_webPlayer_domain_create', 'API.bankerId', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer domain create'+'\n')
        
        number = IniValue('setting', 'API.number')
        str_webDomain = IniValue('setting', 'API.webdomain2')
        list_webDomain = str_webDomain.split(', ')
        list_webDomain2 = []
        for each_webDomain in list_webDomain:
            list_webDomain2.append(each_webDomain+number)
        webDomain2 = ', '.join(list_webDomain2)
        bankerId = IniValue('setting', 'API.bankerId')
        ReplaceIniValue('admin_webPlayer_domain_create', 'API.webDomain[]', webDomain2)
        ReplaceIniValue('admin_webPlayer_domain_create', 'API.bankerId', bankerId)
        self.res_log, result = AdminDomainCreate(self.res_log, 'Banker', webDomain2)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer domain create')
