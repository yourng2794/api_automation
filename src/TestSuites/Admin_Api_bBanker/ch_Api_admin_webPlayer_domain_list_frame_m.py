...
from O2oUtility import WriteLog, WriteDebugLog, IniValue, ReplaceIniValue, AdminDomainListFrame
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer domain list frame m'
        self.CaseNote= ''
        self.TestCaseID= '235'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_domain_list_frame', 'API.webDomain', '')
        ReplaceIniValue('admin_webPlayer_domain_list_frame', 'API.bankerid', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer domain list frame m'+'\n')
        
        number = IniValue('setting', 'API.number')
        str_webDomain2 = IniValue('setting', 'API.webdomain2')
        webDomain2 = str_webDomain2.split(', ')[0]+number
        bankerId = IniValue('setting', 'API.bankerId')
        ReplaceIniValue('admin_webPlayer_domain_list_frame', 'API.webDomain', webDomain2)
        ReplaceIniValue('admin_webPlayer_domain_list_frame', 'API.bankerid', bankerId)
        self.res_log, result, webdomainid2 = AdminDomainListFrame(self.res_log, 'Banker', '', '', 'Mobile')
        
        if(result == True):
            ReplaceIniValue('setting', 'API.webdomainid2', webdomainid2)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer domain list frame m')
