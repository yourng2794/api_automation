...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider payOption payment get'
        self.CaseNote= ''
        self.TestCaseID= '332'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption payment get'+'\n')
        ...
        try:
            ...
            ReplaceIniValue('admin_provider_payOption_payment_get', 'API.bankerId', bankerId)
            ReplaceIniValue('admin_provider_payOption_payment_get', 'API.currency', currency)
            ReplaceIniValue('admin_provider_payOption_payment_get', 'API.payOption', '1')
            
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_provider_payOption_payment_get', False, False)
            ...
            if(return_value):
                ...
                if(json_response[paymentName] == paymentId):
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get paymentId: '+paymentId)
                    result_list.append('PASS')
                else:
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get paymentId: '+json_response[paymentName] +' != '+ paymentId)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_provider_payOption_payment_get... %s' % str(e))
            
        finally:
            self.res_log = WriteDebugLog(self.res_log, print_tab+'---')
        
        try:
            ReplaceIniValue('admin_provider_payOption_payment_get', 'API.payOption', '2')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_provider_payOption_payment_get', False, False)
            WriteLog(print_tab+'Response data: '+ response_data)
            if(return_value):
                self.res_log = WriteDebugLog(self.res_log, print_tab+'- API success (status and response are correct).')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption payment get')
