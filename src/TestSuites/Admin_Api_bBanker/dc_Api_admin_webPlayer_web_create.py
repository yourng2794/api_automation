...
from O2oUtility import WriteLog, WriteDebugLog, IniValue, ReplaceIniValue, AdminWebCreate
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST admin webPlayer web create'
        self.CaseNote= ''
        self.TestCaseID= '240'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_web_create', 'API.bankerId', '')
        ReplaceIniValue('admin_webPlayer_web_create', 'API.webdomainId', '')
        ReplaceIniValue('admin_webPlayer_web_create', 'API.webName', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST admin webPlayer web create'+'\n')
        
        number = IniValue('setting', 'API.number')
        bankerId = IniValue('setting', 'API.bankerId')
        webDomainId2 = IniValue('setting', 'API.webdomainid2')
        webName2 = IniValue('setting', 'API.webname2')+number
        ReplaceIniValue('admin_webPlayer_web_create', 'API.bankerId', bankerId)
        python_dict = {}
        python_dict[webDomainId2] = '1'
        ReplaceIniValue('admin_webPlayer_web_create', 'API.webdomainid', str(python_dict).replace("'",'"').replace(' ',''))
        ReplaceIniValue('admin_webPlayer_web_create', 'API.webName', webName2)
            
        self.res_log, result = AdminWebCreate(self.res_log, 'Banker')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST admin webPlayer web create')
