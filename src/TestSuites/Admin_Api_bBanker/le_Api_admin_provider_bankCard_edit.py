...
class TestCase(BaseCase):
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider bankCard edit'+'\n')
        ...
        try:
            ...
            ReplaceIniValue('admin_provider_bankCard_edit', 'API.bankName', bankname2)
            ReplaceIniValue('admin_provider_bankCard_edit', 'API.branchName', branchname2)
            ReplaceIniValue('admin_provider_bankCard_edit', 'API.bankAcc', bankacc2)
            ReplaceIniValue('admin_provider_bankCard_edit', 'API.accName', accname2)
            ReplaceIniValue('admin_provider_bankCard_edit', 'API.cardId', cardid2)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider bankCard edit')
