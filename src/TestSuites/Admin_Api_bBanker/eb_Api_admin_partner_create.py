...
from O2oUtility import WriteLog, WriteDebugLog, IniValue, ReplaceIniValue, AdminPartnerCreate
...
class TestCase(BaseCase):
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner create'+'\n')
        ...
        typeId2 = IniValue('setting', 'API.typeid2')
        ...
        ReplaceIniValue('admin_partner_create', 'API.ptAcc', webMasterAcc2)
        ReplaceIniValue('admin_partner_create', 'API.ptPwd', webMasterAcc2)
        ReplaceIniValue('admin_partner_create', 'API.ptTypeId', typeId2)
        ReplaceIniValue('admin_partner_create', 'API.ptName', myname)
        ReplaceIniValue('admin_partner_create', 'API.ptParentId', bankerId)
        ReplaceIniValue('admin_partner_create', 'API.webId', webId2)
        ReplaceIniValue('admin_partner_create', 'API.percent', webmaster_percent)
        ReplaceIniValue('admin_partner_create', 'API.agentType', '') # ('' => Banker; 1 => 一般代理; 2 => 末端代理; 3 => SEO代理)
        ReplaceIniValue('admin_partner_create', 'API.currency[]', currency) # Now limit currency is one currency.
        self.res_log, result = AdminPartnerCreate(self.res_log, 'Banker')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner create')
