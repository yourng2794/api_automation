...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider payOption edit get layer frame m'
        self.CaseNote= ''
        self.TestCaseID= '331'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption edit get layer frame m'+'\n')
        ...
        try:
            ...
            ReplaceIniValue('admin_provider_payOption_edit_get_layer_frame', 'API.webId', webId2)
            ReplaceIniValue('admin_provider_payOption_edit_get_layer_frame', 'API.payOptionId', payOptionId2)
            ReplaceIniValue('admin_provider_payOption_edit_get_layer_frame', 'API.bankerId', '')
            ReplaceIniValue('admin_provider_payOption_edit_get_layer_frame', 'API.api_path', 'api/provider/payOption/edit/get/layer/frame/m')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_provider_payOption_edit_get_layer_frame', False, False)
            ...
            if(return_value):
                ...
                for selected_value in selected_values:
                    if(selected_value.get('selected') == ''):
                        if(selected_value.get('value') == payOptionId2):
                            result_list2.append('PASS')
                            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get payOptionId2: '+payOptionId2)
                        else:
                            result_list2.append('FAIL')
                            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get payOptionId2: '+selected_value.get('value')+' != '+payOptionId2)
                            
                        if(str(selected_value.string).replace('\n','').replace(' ','') == payOptionName2):
                            result_list2.append('PASS')
                            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get payOptionName2: '+payOptionName2)
                        else:
                            result_list2.append('FAIL')
                            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get payOptionName2: '+str(selected_value.string).replace('\n','').replace(' ','')+' != '+payOptionName2)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_provider_payOption_edit_get_layer_frame_m... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption edit get layer frame m')
