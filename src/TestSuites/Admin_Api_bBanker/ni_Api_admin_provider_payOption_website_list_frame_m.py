...
class TestCase(BaseCase):
    ...
    def run(self, copy_result):
        ...
        result_list2 = []
        result_list3 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption website list frame m'+'\n')
        ...
        try:
            ...
            ReplaceIniValue('admin_provider_payOption_website_list', 'API.bankerId', bankerId)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption website list frame m')
