...
class TestCase(BaseCase):
    ...
    def run(self, copy_result):
        ...
        print_tab = '\n\t\t'
        try:
            userId2 = IniValue('setting', 'API.userid2')
            userAcc2 = IniValue('setting', 'API.playeracc2')
            depTitleId2 = IniValue('setting', 'API.deptitleid2')
            depId2 = IniValue('setting', 'API.depid2')
            ReplaceIniValue('admin_org_employee_list_single', 'API.userId', userId2)
            ...
            if(return_value):
                ...
                if(json_response['user_acc'] == userAcc2):
                    result_list2.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get userAcc2: '+userAcc2)
                else:
                    result_list2.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get userAcc2')
                
                if(json_response['dp_sid'] == depTitleId2):
                    result_list2.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get depTitleId2: '+depTitleId2)
                else:
                    result_list2.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get depTitleId2')
                
                if(json_response['og_sid'] == depId2):
                    result_list2.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get depId2: '+depId2)
                else:
                    result_list2.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get depId2')
