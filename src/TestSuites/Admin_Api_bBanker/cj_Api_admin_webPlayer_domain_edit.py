...
from O2oUtility import WriteLog, WriteDebugLog, IniValue, ReplaceIniValue, AdminDomainEdit
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer domain edit'
        self.CaseNote= ''
        self.TestCaseID= '237'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_domain_edit', 'API.webDomainId', '')
        ReplaceIniValue('admin_webPlayer_domain_edit', 'API.webDomain', '')
        ReplaceIniValue('admin_webPlayer_domain_edit', 'API.status', '')
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer domain edit'+'\n')
        ...
        ReplaceIniValue('admin_webPlayer_domain_edit', 'API.webDomainId', webDomainId2)
        ReplaceIniValue('admin_webPlayer_domain_edit', 'API.webDomain', webDomain2)
        ReplaceIniValue('admin_webPlayer_domain_edit', 'API.status', '0')
        self.res_log, result = AdminDomainEdit(self.res_log, 'Banker')
        result_list.append(result)
        
        ReplaceIniValue('admin_webPlayer_domain_edit', 'API.status', '1')
        self.res_log, result = AdminDomainEdit(self.res_log, 'Banker')
        result_list.append(result)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer domain edit')
