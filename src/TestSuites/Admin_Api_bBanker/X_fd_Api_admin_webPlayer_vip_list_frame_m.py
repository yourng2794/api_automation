...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer vip list frame m'
        self.CaseNote= ''
        self.TestCaseID= '264'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer vip list frame m'+'\n')
        ...
        try:
            ...
            if(return_value):
                ...
                for soup_vipid in soup_vipids:
                    if(str(soup_vipid.string).replace('\n','').replace(' ','') == vipname2):
                        ...
                        vipid2 = soup_vipid.get('id')
                        ReplaceIniValue('setting', 'API.vipid2', vipid2)
                        self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get vipid2: '+vipid2)
                    else:
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_vip_list_frame_m... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer vip list frame m')
