...
class TestCase(BaseCase):
    ...
    def run(self, copy_result):
        ...
        try:
            ...
            titleId2 = IniValue('setting', 'API.deptitleid2')
            userId2 = IniValue('setting', 'API.userid2')
            userAcc2 = IniValue('setting', 'API.playeracc2')
            ReplaceIniValue('admin_org_employee_list_desc_frame', 'API.depId', depId2)
            ReplaceIniValue('admin_org_employee_list_desc_frame', 'API.titleId', titleId2)
            ReplaceIniValue('admin_org_employee_list_desc_frame', 'API.userAcc', userAcc2)
            ...
            if(return_value):
                ...
                user_Id2 = soup.find('a', attrs={'href':'organize_mem_edit.html'}).get('id')
                if(user_Id2 == userId2):
                    result_list2.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get userId2: '+userId2)
                else:
                    result_list2.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get userId2')
