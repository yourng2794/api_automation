...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer web vip group edit frame m'
        self.CaseNote= ''
        self.TestCaseID= '279'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer web vip group edit frame m'+'\n')
        ...
        try:
            ...
            ReplaceIniValue('admin_webPlayer_web_vip_group_edit_frame', 'API.webId', webId2)
            ReplaceIniValue('admin_webPlayer_web_vip_group_edit_frame', 'API.vipGroupId', vipGroupId2)
            ReplaceIniValue('admin_webPlayer_web_vip_group_edit_frame', 'API.api_path', 'api/webPlayer/web/vip/group/edit/frame/m')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_web_vip_group_edit_frame', False, False)
...
#                 soup_vipNames = soup.find_all("div")
#                 for soup_vipName in soup_vipNames:
#                     if(soup_vipName.string == vipName):
#                         self.res_log = WriteDebugLog(self.res_log, print_tab+'Success get vipName: '+vipName)
#                         if(soup_vipName.get('id') == vipId):
#                             result_list.append('PASS')
                if((vipId2 in response_data) and (vipName2 in response_data) and (webDomain2 in response_data)):
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_web_vip_group_edit_frame_m... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer web vip group edit frame m')
