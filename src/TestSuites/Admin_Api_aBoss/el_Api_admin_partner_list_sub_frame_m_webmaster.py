...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST partner list sub frame m webmaster'
        self.CaseNote= ''
        self.TestCaseID= '382'
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner list sub frame m webmaster'+'\n')
        
        number = IniValue('setting', 'API.number')
        bankerId = IniValue('setting', 'API.bankerId')
        ReplaceIniValue('admin_partner_list_sub_frame', 'API.parentId', bankerId)
        webMasterAcc = IniValue('setting', 'API.webMasterAcc')+number
        
        self.res_log, result, webMasterId = AdminPartnerListSubFrame(self.res_log, 'Boss', webMasterAcc, 'Mobile')
        
        if(result == True):
            ReplaceIniValue('setting', 'API.webMasterId', webMasterId)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner list sub frame m webmaster')
