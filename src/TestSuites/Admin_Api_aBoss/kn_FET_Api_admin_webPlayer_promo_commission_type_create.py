...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[FET] API POST webPlayer promo commission type create'
        self.CaseNote= ''
        self.TestCaseID= '630'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.bankerId', '')
    ...
    def run(self, copy_result):
        ...
        try:
            ...
            ReplaceIniValue('admin_webPlayer_promo_commission_type_create', 'API.bankerId', bankerId)
            ReplaceIniValue('admin_webPlayer_promo_commission_type_create', 'API.webCommissionName', commissionTypeName)
            ReplaceIniValue('admin_webPlayer_promo_commission_type_create', 'API.commissionTimes', '100') # 打碼倍數
            ReplaceIniValue('admin_webPlayer_promo_commission_type_create', 'API.receiveTimesDaily', '200') # 每日領取次數
            ReplaceIniValue('admin_webPlayer_promo_commission_type_create', 'API.totalBet', '300') # 有效總投注
            ReplaceIniValue('admin_webPlayer_promo_commission_type_create', 'API.promotionLimit', '400') # 優惠上限
            ReplaceIniValue('admin_webPlayer_promo_commission_type_create', 'API.maxReceiveAmount', '500') # 單日最高領取金額
            ReplaceIniValue('admin_webPlayer_promo_commission_type_create', 'API.singleMinReceiveAmount', '600') # 單次最低領取金額
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_promo_commission_type_create', False, False)
            ...
            if(return_value):
                ...
                if(json_response['Error_code'] == 'E23' and json_response['Error_message'] == 'Error Parameters'):
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success (Error_code, Error_message): (E23, Error Parameters)')
                    result_list.append('PASS')
                else:
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail (Error_code, Error_message): ('+json_response['Error_code']+', '+json_response['Error_message']+')')
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_promo_commission_type_create... %s' % str(e))
