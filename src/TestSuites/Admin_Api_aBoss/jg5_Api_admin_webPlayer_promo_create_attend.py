...
from O2oUtility import AdminWebPromoCreate
...
#         ReplaceIniValue('admin_webPlayer_promo_create', 'API.promoName', '')
#         ReplaceIniValue('admin_webPlayer_promo_create', 'API.webId', '')
#         ReplaceIniValue('admin_webPlayer_promo_create', 'API.vipId[]', '')
#         ReplaceIniValue('admin_webPlayer_promo_create', 'API.promoType', '')
#         ReplaceIniValue('admin_webPlayer_promo_create', 'API.promoAudit', '')
#         ReplaceIniValue('admin_webPlayer_promo_create', 'API.rewards', '')
#         ReplaceIniValue('admin_webPlayer_promo_create', 'API.condition', '') 
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo create'+'\n')
        
        promoName = IniValue('setting', 'API.promoname')+'簽到'
        webId = IniValue('setting', 'API.webid')
        vipId = IniValue('setting', 'API.vipid')
        condition5 = IniValue('setting', 'API.condition5')
        ReplaceIniValue('admin_webPlayer_promo_create', 'API.promoName', promoName)
        ReplaceIniValue('admin_webPlayer_promo_create', 'API.webId', webId)
        ReplaceIniValue('admin_webPlayer_promo_create', 'API.vipId[]', vipId)
        ReplaceIniValue('admin_webPlayer_promo_create', 'API.promoType', '5') # 1 normal, 2 birth, 3 save, 4 friend, 5 attend
        ReplaceIniValue('admin_webPlayer_promo_create', 'API.promoAudit', '10') # 2 multiple bet
        ReplaceIniValue('admin_webPlayer_promo_create', 'API.rewards', '10.00') # 3 give money
        ReplaceIniValue('admin_webPlayer_promo_create', 'API.condition', condition5) # 4 Change to attend format 每日最低存款金額=>depositAmountMin,累計簽到天數=>signDays,達到天數後可領取=>signReceiveAmount
        now_time= datetime.now()
        start_time = now_time+ timedelta(hours=24)
        end_time = start_time+ timedelta(hours=24) # time period: tomorrow ~ the day after tomorrow
        time = str(start_time.strftime("%Y-%m-%d"))+' ~ '+str(end_time.strftime("%Y-%m-%d"))
        ReplaceIniValue('admin_webPlayer_promo_create', 'API.time', time)
        
        self.res_log, result= AdminWebPromoCreate(self.res_log, 'Boss')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo create')
