...
class TestCase(BaseCase):
    ...
    def run(self, copy_result):
        ...
        try:
            ...
            despositid = IniValue('setting', 'API.despositid')
            withdrawid = IniValue('setting', 'API.withdrawid')
            ReplaceIniValue('admin_provider_payOption_edit_get_withdraw', 'API.payOptionId', payOptionId)
...
#                 print(list(json_response.keys()))
                for element in list(json_response.keys()):
                    p_id = json_response.get(element)[0]
                    if(p_id in withdrawid):
                        result_list.append('PASS')
                    else:
