...
from O2oUtility import AdminPartnerTypeListFrame
...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST partner type list frame m'
        self.CaseNote= ''
        self.TestCaseID= '164'
...
#         ReplaceIniValue('admin_partner_type_list_frame', 'API.searchType', '')
#         ReplaceIniValue('admin_partner_type_list_frame', 'API.search', '')
#         ReplaceIniValue('admin_partner_type_list_frame', 'API.listType', '')
#         ReplaceIniValue('admin_partner_type_list_frame', 'API.page', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner type list frame m'+'\n')
        
        number = IniValue('setting', 'API.number')
        typeName = IniValue('setting', 'API.typename')+number
        
        self.res_log, result, typeId = AdminPartnerTypeListFrame(self.res_log, 'Boss', typeName, 'Mobile')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner type list frame m')
