...
from O2oUtility import AdminOrgJobtitleCreate
...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST org. jobtitle create'
        self.CaseNote= ''
        self.TestCaseID= '190'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_org_jobtitle_create', 'API.depId', '')
        ReplaceIniValue('admin_org_jobtitle_create', 'API.title', '')
        ReplaceIniValue('admin_org_jobtitle_create', 'API.descrip', '')
#         ReplaceIniValue('admin_org_jobtitle_create', 'API.permission', '{}')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. jobtitle create'+'\n')
        
        number = IniValue('setting', 'API.number')
        depId = IniValue('setting', 'API.depid')
        depTitle = IniValue('setting', 'API.deptitle')+number
        ReplaceIniValue('admin_org_jobtitle_create', 'API.depId', depId)
        ReplaceIniValue('admin_org_jobtitle_create', 'API.title', depTitle)
        ReplaceIniValue('admin_org_jobtitle_create', 'API.descrip', '')
        
        self.res_log, result = AdminOrgJobtitleCreate(self.res_log, 'Boss')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. jobtitle create')
