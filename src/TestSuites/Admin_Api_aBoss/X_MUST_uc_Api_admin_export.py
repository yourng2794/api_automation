...
from O2oUtility import AdminExport
...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST export'
        self.CaseNote= ''
        self.TestCaseID= '207'
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST export'+'\n')
#         list_type = '"MANUAL_INCR", "MANUAL_DECR", "THIRD_PARTY_INCR", "THIRD_PARTY_DECR", "BANK_INCR", "BANK_DECR", "TRANSFER_WALLET_DECR", "TRANSFER_WALLET_INCR", "SINGLE_WALLET_DECR", "SINGLE_WALLET_INCR", "PLAYER_COMM_INCR", "DEPOSIT_BONUS_INCR", "PROMOTE_BONUS_INCR", "bet", "win", "revoke", "KA_slot"'
        ...
        ReplaceIniValue('admin_export', 'API.bankerId', '67d8055d44df916e03024e3fbfc83a2c')
        ReplaceIniValue('admin_export', 'API.webId', '0b2ab3d05417542282220cd5af960b86')
        ReplaceIniValue('admin_export', 'API.currency', currency)
        ReplaceIniValue('admin_export', 'API.startTime', '2019-03-10')
        ReplaceIniValue('admin_export', 'API.endTime', '2019-05-15')
        ReplaceIniValue('admin_export', 'API.type[]', list_type)
        ReplaceIniValue('admin_export', 'API.export', '1')
        ReplaceIniValue('admin_export', 'API.page', '3')
        
        self.res_log, result = AdminExport(self.res_log, 'Boss')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST export')
