...
from O2oUtility import AdminWebProviderGet
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer web provider get'
        self.CaseNote= ''
        self.TestCaseID= '633'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_web_provider_get', 'API.providerId', '')
        ReplaceIniValue('admin_webPlayer_web_provider_get', 'API.webId', '')
        ReplaceIniValue('admin_webPlayer_web_provider_get', 'API.page', '')
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer web provider get'+'\n')
        
        number = IniValue('setting', 'API.number')
        provider_ids = IniValue('setting', 'API.provider_ids')
        provider_id_list = provider_ids.split(', ')
        webId = IniValue('setting', 'API.webId')
        webName = IniValue('setting', 'API.webName')+number
        ReplaceIniValue('admin_webPlayer_web_provider_get', 'API.webId', webId)
        ReplaceIniValue('admin_webPlayer_web_provider_get', 'API.lang', 'zh')
        for provider_id in provider_id_list:
            ReplaceIniValue('admin_webPlayer_web_provider_get', 'API.providerId', provider_id)
            
            self.res_log, result, provider_name, number_of_game = AdminWebProviderGet(self.res_log, 'Boss', webName)
            result_list.append(result)
            result_list2.append(provider_name+' = '+number_of_game)
        
        WriteLog(result_list)
        for each_item in result_list2:
            WriteLog(each_item+'\n')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer web provider get')
