        ...
        try:
            ...
            if(return_value):
                ...
                if(json_response['payoption_name'] == payOptionName):
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get payoption_name: '+payOptionName)
                    result_list.append('PASS')
                else:
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get payoption_name: '+json_response['payoption_name']+ ' != '+ payOptionName)
                    result_list.append('FAIL')
                    
                if(json_response2.get(webId) == webName):
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get webName: '+webName)
                    result_list.append('PASS')
                else:
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get webName: '+json_response2.get(webId)+ ' != '+ webName)
                    result_list.append('FAIL')
