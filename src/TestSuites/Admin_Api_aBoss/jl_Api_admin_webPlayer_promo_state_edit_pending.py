...
from O2oUtility import AdminWebPromoStateEdit
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer promo state edit pending'
        self.CaseNote= ''
        self.TestCaseID= '110'
...
#         ReplaceIniValue('admin_webPlayer_promo_state_edit', 'API.promoId', '')
#         ReplaceIniValue('admin_webPlayer_promo_state_edit', 'API.enable', '')
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo state edit pending'+'\n')
        
        promoId = IniValue('setting', 'API.promoid')
        ReplaceIniValue('admin_webPlayer_promo_state_edit', 'API.promoId', promoId)
        ReplaceIniValue('admin_webPlayer_promo_state_edit', 'API.enable', '0') # enable = 0 (停用), enable = 1 (进行中)
        self.res_log, result= AdminWebPromoStateEdit(self.res_log, 'Boss', '停用')
        result_list.append(result)
        
        ReplaceIniValue('admin_webPlayer_promo_state_edit', 'API.enable', '1')
        self.res_log, result= AdminWebPromoStateEdit(self.res_log, 'Boss', '未开始')
        result_list.append(result)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo state edit pending')
