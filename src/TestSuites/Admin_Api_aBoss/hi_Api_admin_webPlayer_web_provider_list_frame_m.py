...
from O2oUtility import AdminWebProviderListFrame
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer web provider list frame m'
        self.CaseNote= ''
        self.TestCaseID= '632'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_web_provider_list_frame', 'API.webId', '')
        ReplaceIniValue('admin_webPlayer_web_provider_list_frame', 'API.page', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer web provider list frame m'+'\n')
        
        webId = IniValue('setting', 'API.webId')
        ReplaceIniValue('admin_webPlayer_web_provider_list_frame', 'API.webId', webId)
        ReplaceIniValue('admin_webPlayer_web_provider_list_frame', 'API.page', '')
        
        self.res_log, result, provider_ids = AdminWebProviderListFrame(self.res_log, 'Boss', 'Mobile')
        
        if(result == True):
            ReplaceIniValue('setting', 'API.provider_ids', provider_ids)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer web provider list frame m')
