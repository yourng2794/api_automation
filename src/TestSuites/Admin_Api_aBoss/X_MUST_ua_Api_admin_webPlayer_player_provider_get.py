...
from O2oUtility import AdminWebPlayerProviderGet
...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer player provider get'
        self.CaseNote= ''
        self.TestCaseID= '207'
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer player provider get'+'\n')
        list_type = ['slot', 'sport', 'lived', 'lottery', 'fish', 'poker','esport']
        
        self.res_log, result = AdminWebPlayerProviderGet(self.res_log, 'Boss', list_type)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer player provider get')
