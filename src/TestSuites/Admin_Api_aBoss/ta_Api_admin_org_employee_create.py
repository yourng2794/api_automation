...
from O2oUtility import AdminOrgEmployeeCreate
...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST org. employee create'
        self.CaseNote= ''
        self.TestCaseID= '196'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_org_employee_create', 'API.userAcc', '')
        ReplaceIniValue('admin_org_employee_create', 'API.userPwd', '')
        ReplaceIniValue('admin_org_employee_create', 'API.depId', '')
        ReplaceIniValue('admin_org_employee_create', 'API.titleId', '')
        ReplaceIniValue('admin_org_employee_create', 'API.userNote', '')
        ReplaceIniValue('admin_org_employee_create', 'API.website[]', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. employee create'+'\n')
        
        number = IniValue('setting', 'API.number')
        userAcc = IniValue('setting', 'API.employeeacc1')+str(number)
        depId = IniValue('setting', 'API.depid')
        titleId = IniValue('setting', 'API.deptitleid')
        webId = IniValue('setting', 'API.webId')
        ReplaceIniValue('admin_org_employee_create', 'API.userAcc', userAcc)
        ReplaceIniValue('admin_org_employee_create', 'API.userPwd', userAcc)
        ReplaceIniValue('admin_org_employee_create', 'API.depId', depId)
        ReplaceIniValue('admin_org_employee_create', 'API.titleId', titleId)
        ReplaceIniValue('admin_org_employee_create', 'API.userNote', '')
        ReplaceIniValue('admin_org_employee_create', 'API.website[]', webId)
        
        self.res_log, result = AdminOrgEmployeeCreate(self.res_log, 'Boss')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. employee create')
