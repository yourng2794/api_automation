...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST partner create agent'
        self.CaseNote= ''
        self.TestCaseID= '168'
...
#         ReplaceIniValue('admin_partner_create', 'API.permission', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner create agent'+'\n')
        ...
        ptParentId = IniValue('setting', 'API.webMasterId')
        typeId = IniValue('setting', 'API.typeId')
        webId = IniValue('setting', 'API.webId')
        agent1_percent = IniValue('setting', 'API.agent1_percent')
        currency = IniValue('setting', 'API.currency')
        ReplaceIniValue('admin_partner_create', 'API.ptAcc', agentAcc)
        ReplaceIniValue('admin_partner_create', 'API.ptPwd', agentAcc)
        ReplaceIniValue('admin_partner_create', 'API.ptParentId', ptParentId)
        ReplaceIniValue('admin_partner_create', 'API.ptTypeId', typeId)
        ReplaceIniValue('admin_partner_create', 'API.ptName', myname)
        ReplaceIniValue('admin_partner_create', 'API.webId', webId)
        ReplaceIniValue('admin_partner_create', 'API.percent', agent1_percent)
        ReplaceIniValue('admin_partner_create', 'API.currency[]', currency) # Now limit currency is one currency.
        ReplaceIniValue('admin_partner_create', 'API.agentType', '1') # 1 => 一般代理; 2 => 末端代理; 3 => SEO代理
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner create agent')
