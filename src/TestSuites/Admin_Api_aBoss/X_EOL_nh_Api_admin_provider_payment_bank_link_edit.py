...
#         list_banknames = []
        ...
        try:
            ...
            ReplaceIniValue('admin_provider_payment_bank_get', 'API.bankerId', partnerId)
            ...
            if(return_value):
                ...
                if(str(json_response[bankName]) == '0'):
                    result_list.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get (bankName, value): '+bankName+', 0')
                else:
...
#             json_dict['withdraw'][currency]['0'] = list_banknames[0]
        ...
        try:
            ...
            if(return_value):
                ...
                bankName = IniValue('setting', 'API.bankname')
                json_response = json.loads(response_data)
                if(str(json_response[bankName]) == '1'):
                    result_list.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get (bankName, value): '+bankName+', 1')
                else:
                    result_list.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get (bankName, value)')
                self.res_log = WriteDebugLog(self.res_log, print_tab+'---')
