...
from O2oUtility import AdminOrgJobtitleEdit
...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST org. jobtitle edit'
        self.CaseNote= ''
        self.TestCaseID= '195'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_org_jobtitle_edit', 'API.titleId', '')
        ReplaceIniValue('admin_org_jobtitle_edit', 'API.descrip', '')
        ReplaceIniValue('admin_org_jobtitle_edit', 'API.title', '')
#         ReplaceIniValue('admin_org_jobtitle_edit', 'API.permission', '{}')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. jobtitle edit'+'\n')
        
        number = IniValue('setting', 'API.number')
        depTitleId = IniValue('setting', 'API.deptitleid')
        ReplaceIniValue('admin_org_jobtitle_edit', 'API.titleId', depTitleId)
        ReplaceIniValue('admin_org_jobtitle_edit', 'API.descrip', 'After Edit')
        depTitle = IniValue('setting', 'API.deptitle')+number
        ReplaceIniValue('admin_org_jobtitle_edit', 'API.title', depTitle)
        
        self.res_log, result = AdminOrgJobtitleEdit(self.res_log, 'Boss')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. jobtitle edit')
