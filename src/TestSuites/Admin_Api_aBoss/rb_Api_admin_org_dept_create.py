...
from O2oUtility import AdminOrgDeptCreate
...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST org. dept. create'
        self.CaseNote= ''
        self.TestCaseID= '183'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_org_dept_create', 'API.name', '')
        ReplaceIniValue('admin_org_dept_create', 'API.descrip', '')
        ReplaceIniValue('admin_org_dept_create', 'API.permission', '{}')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. dept. create'+'\n')
        
        number = IniValue('setting', 'API.number')
        depname = IniValue('setting', 'API.depname')+number
        desc = IniValue('setting', 'API.myname')+number
        ReplaceIniValue('admin_org_dept_create', 'API.name', depname)
        ReplaceIniValue('admin_org_dept_create', 'API.descrip', desc)
        
        self.res_log, result = AdminOrgDeptCreate(self.res_log, 'Boss')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. dept. create')
