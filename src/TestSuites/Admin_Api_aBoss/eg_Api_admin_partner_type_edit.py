...
from O2oUtility import AdminPartnerTypeEdit
...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST partner type edit'
        self.CaseNote= ''
        self.TestCaseID= '166'
...
#         ReplaceIniValue('admin_partner_type_edit', 'API.typeId', '')
#         ReplaceIniValue('admin_partner_type_edit', 'API.typeName', '')
#         ReplaceIniValue('admin_partner_type_edit', 'API.permission', '')
#         ReplaceIniValue('admin_partner_type_edit', 'API.currency', '')
#         ReplaceIniValue('admin_partner_type_edit', 'API.payment', '')
#         ReplaceIniValue('admin_partner_type_edit', 'API.provider', '')
#         ReplaceIniValue('admin_partner_type_edit', 'API.stopbet', '')
#         ReplaceIniValue('admin_partner_type_edit', 'API.handicap', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner type edit'+'\n')
        
        result_list = []
        number = IniValue('setting', 'API.number')
        typeId = IniValue('setting', 'API.typeid')
        typeName = IniValue('setting', 'API.typename')+number
        ReplaceIniValue('admin_partner_type_edit', 'API.typeId', typeId)
        ReplaceIniValue('admin_partner_type_edit', 'API.typeName', typeName)
        currency = IniValue('admin_partner_type_edit', 'API.currency[]')
        ReplaceIniValue('admin_partner_type_edit', 'API.currency[]', currency.replace(', HKD',''))
        
        self.res_log, result = AdminPartnerTypeEdit(self.res_log, 'Boss')
        result_list.append(result)
        
        #------------------------------------------------------------
        currency = IniValue('admin_partner_type_edit', 'API.currency[]')
        ReplaceIniValue('admin_partner_type_edit', 'API.currency[]', currency+', HKD')
        self.res_log, result = AdminPartnerTypeEdit(self.res_log, 'Boss')
        result_list.append(result)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner type edit')
