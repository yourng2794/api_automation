...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer promo state edit active'
        self.CaseNote= ''
        self.TestCaseID= '110'
...
#         ReplaceIniValue('admin_webPlayer_promo_state_edit', 'API.enable', '')
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo state edit active'+'\n')
        ...
        self.res_log, result= AdminWebPromoStateEdit(self.res_log, 'Boss', '进行中')
        result_list.append(result)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo state edit active')
