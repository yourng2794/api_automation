...
#         ReplaceIniValue('admin_webPlayer_promo_type_create', 'API.reward', '')
    ...
    def run(self, copy_result):
        ...
        promoTypeName = 'QA生日範本'+number
#         promoTypeName = promoTypeName+today_time.strftime("%m%d_%H%M%S")
        ReplaceIniValue('setting', 'API.promoTypeName2', promoTypeName)
        ...
        ReplaceIniValue('admin_webPlayer_promo_type_create', 'API.promoType', '2')
        ReplaceIniValue('admin_webPlayer_promo_type_create', 'API.promoAudit', '10.00')
        ReplaceIniValue('admin_webPlayer_promo_type_create', 'API.condition', condition2) # 4 Change to birth format 當月存款金額=>currentDepositAmount,放寬天數=>relaxationDays
