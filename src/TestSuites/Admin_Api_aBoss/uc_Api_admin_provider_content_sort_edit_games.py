        ...
        for providerGame in providerGames.split(', '):
            ...
            if(not(gmtype == 'lived' or gmtype == 'sport' or gmtype == 'esport')):
                ...
                if(('gameid' in response_data) and (len(json_response) > 2)):
                    list_random = random.sample(range(0, len(json_response)-1), 2)
                    first_key = list_random[0]
                    second_key = list_random[1]
                    json_random1 = json_response[first_key]
                    json_random1['providername'] = providerGame
                    json_random1.get('gamename')
                    json_random2 = json_response[second_key]
                    json_random2['providername'] = providerGame
                ...
                elif(('gameid' in response_data) and (len(json_response) == 2)):
                    json_random1 = json_response[0]
                    json_random2 = json_response[1]
                    json_random1['providername'] = providerGame
                    json_random1.get('gamename')
                    json_random2['providername'] = providerGame
                    json_random2.get('gamename')
