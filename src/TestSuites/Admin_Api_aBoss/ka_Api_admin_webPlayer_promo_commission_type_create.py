...
#         ReplaceIniValue('admin_webPlayer_promo_commission_type_create', 'API.singleMinReceiveAmount', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+today_time.strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo commission type create'+'\n')
        
        bankerId = IniValue('setting', 'API.bankerId')
        commissionTypeName = 'SQArake_'
        commissionTypeName = commissionTypeName+today_time.strftime("%m%d_%H%M%S")
        ReplaceIniValue('setting', 'API.commissionTypeName', commissionTypeName)
        ...
        self.res_log, result = AdminWebPromoCommissionTypeCreate(self.res_log, 'Boss')
