...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer promo type list frame'
        self.CaseNote= ''
        self.TestCaseID= '111'
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo type list frame'+'\n')
        list_result = []
        ...
        ReplaceIniValue('admin_webPlayer_promo_type_list_frame', 'API.page', '')
        self.res_log, result, promoTypeId = AdminWebPromoTypeListFrame(self.res_log, 'Boss', promoTypeName, 'Web')
        
        promoTypeName2 = IniValue('setting', 'API.promoTypeName2')
        ReplaceIniValue('admin_webPlayer_promo_type_list_frame', 'API.promoType', '2')
        self.res_log, result2, promoTypeId2 = AdminWebPromoTypeListFrame(self.res_log, 'Boss', promoTypeName2, 'Web')
        
        promoTypeName3 = IniValue('setting', 'API.promoTypeName3')
        ReplaceIniValue('admin_webPlayer_promo_type_list_frame', 'API.promoType', '3')
        self.res_log, result3, promoTypeId3 = AdminWebPromoTypeListFrame(self.res_log, 'Boss', promoTypeName3, 'Web')
        
        promoTypeName4 = IniValue('setting', 'API.promoTypeName4')
        ReplaceIniValue('admin_webPlayer_promo_type_list_frame', 'API.promoType', '4')
        self.res_log, result4, promoTypeId4 = AdminWebPromoTypeListFrame(self.res_log, 'Boss', promoTypeName4, 'Web')
        
        promoTypeName5 = IniValue('setting', 'API.promoTypeName5')
        ReplaceIniValue('admin_webPlayer_promo_type_list_frame', 'API.promoType', '5')
        self.res_log, result5, promoTypeId5 = AdminWebPromoTypeListFrame(self.res_log, 'Boss', promoTypeName5, 'Web')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo type list frame')
