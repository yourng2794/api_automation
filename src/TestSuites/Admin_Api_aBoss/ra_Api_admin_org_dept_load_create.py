...
from O2oUtility import AdminOrgDeptLoadCreate
...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST org. dept. load create'
        self.CaseNote= ''
        self.TestCaseID= '180'
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. dept. load create'+'\n')
        
        self.res_log, result, str_permission_final = AdminOrgDeptLoadCreate(self.res_log, 'Boss')
#         json_data = ReplaceAllJsonValue(str_permission_final, 'accCont06|dm', 0)
        if(result == True):
            ReplaceIniValue('admin_org_dept_create', 'API.permission', str_permission_final)
            ReplaceIniValue('admin_org_dept_edit', 'API.permission', str_permission_final)
            ReplaceIniValue('admin_org_jobtitle_create', 'API.permission', str_permission_final)
            ReplaceIniValue('admin_org_jobtitle_edit', 'API.permission', str_permission_final)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. dept. load create')
