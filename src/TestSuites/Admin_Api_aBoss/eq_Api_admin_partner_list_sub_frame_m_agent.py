...
from O2oUtility import AdminPartnerListSubFrame
...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST partner list sub frame m agent'
        self.CaseNote= ''
        self.TestCaseID= '382'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_partner_list_sub_frame', 'API.parentId', '')
        ReplaceIniValue('admin_partner_list_sub_frame', 'API.searchType', '')
        ReplaceIniValue('admin_partner_list_sub_frame', 'API.search', '')
        ReplaceIniValue('admin_partner_list_sub_frame', 'API.page', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner list sub frame m agent'+'\n')
        
        number = IniValue('setting', 'API.number')
        agentAcc = IniValue('setting', 'API.agentAcc1')+number
        webMasterId = IniValue('setting', 'API.webMasterId')
        ReplaceIniValue('admin_partner_list_sub_frame', 'API.parentId', webMasterId)
        
        self.res_log, result, agentId = AdminPartnerListSubFrame(self.res_log, 'Boss', agentAcc, 'Mobile')
        
        if(result == True):
            ReplaceIniValue('setting', 'API.agentId', agentId)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner list sub frame m agent')
