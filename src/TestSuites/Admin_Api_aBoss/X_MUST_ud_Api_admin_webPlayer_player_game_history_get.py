...
from O2oUtility import AdminWebPlayerGameHistoryGet
...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer player game history get'
        self.CaseNote= ''
        self.TestCaseID= '207'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer player game history get'+'\n')
#         list_type = '"MANUAL_INCR", "MANUAL_DECR", "THIRD_PARTY_INCR", "THIRD_PARTY_DECR", "BANK_INCR", "BANK_DECR", "TRANSFER_WALLET_DECR", "TRANSFER_WALLET_INCR", "SINGLE_WALLET_DECR", "SINGLE_WALLET_INCR", "PLAYER_COMM_INCR", "DEPOSIT_BONUS_INCR", "PROMOTE_BONUS_INCR", "bet", "win", "revoke", "KA_slot"'
        list_type = "MANUAL_INCR"
        
        partnerId = IniValue('setting', 'API.partnerId')
        webId = IniValue('setting', 'API.webId')
        currency = IniValue('setting', 'API.currency')
        ReplaceIniValue('admin_webPlayer_player_game_history_get', 'API.bankerId', '67d8055d44df916e03024e3fbfc83a2c')
        ReplaceIniValue('admin_webPlayer_player_game_history_get', 'API.webId', '0b2ab3d05417542282220cd5af960b86')
        ReplaceIniValue('admin_webPlayer_player_game_history_get', 'API.currency', currency)
        ReplaceIniValue('admin_webPlayer_player_game_history_get', 'API.startTime', '2019-03-10')
        ReplaceIniValue('admin_webPlayer_player_game_history_get', 'API.endTime', '2019-05-15')
        ReplaceIniValue('admin_webPlayer_player_game_history_get', 'API.type[]', list_type)
        ReplaceIniValue('admin_webPlayer_player_game_history_get', 'API.export', '0')
        ReplaceIniValue('admin_webPlayer_player_game_history_get', 'API.page', '3')
        
        self.res_log, result = AdminWebPlayerGameHistoryGet(self.res_log, 'Boss')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer player game history get')
