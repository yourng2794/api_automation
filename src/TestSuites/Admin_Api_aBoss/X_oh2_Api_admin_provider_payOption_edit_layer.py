...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider payOption edit layer'
        self.CaseNote= ''
        self.TestCaseID= '147'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption edit layer'+'\n')
        
        print_tab = '\n\t\t'
        try:
            webId = IniValue('setting', 'API.webId')
            layerIds = IniValue('setting', 'API.layerId')
            payOptionId = IniValue('setting', 'API.payOptionId')
            ReplaceIniValue('admin_provider_payOption_edit_layer', 'API.webId', webId)
            python_dict = {}
            for layerId in layerIds.split(', '):
                python_dict[layerId] = payOptionId
            print(str(python_dict).replace("'",'"').replace(' ',''))
            ReplaceIniValue('admin_provider_payOption_edit_layer', 'API.layerPay', str(python_dict).replace("'",'"').replace(' ',''))
            
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_provider_payOption_edit_layer', True, False)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_provider_payOption_edit_layer... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption edit layer')
