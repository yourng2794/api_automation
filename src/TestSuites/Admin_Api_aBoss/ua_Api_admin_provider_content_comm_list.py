...
from O2oUtility import AdminProviderContentCommList
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider content comm list'
        self.CaseNote= ''
        self.TestCaseID= '173'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_provider_content_comm_list', 'API.type', '')
    ...
    def run(self, copy_result):
        ...
        list_providerGame = []
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider content comm list'+'\n')
        
        gameType = IniValue('setting', 'TestData.gameType')
        list_gameType = gameType.split(', ')
        for each_gameType in list_gameType:
            if(each_gameType == 'esport'):
                break
            
            ReplaceIniValue('admin_provider_content_comm_list', 'API.type', each_gameType)
            self.res_log, result, list_game_category, list_providerGame = AdminProviderContentCommList(self.res_log, 'Boss', list_providerGame)
            ReplaceIniValue('setting', 'GameType.'+each_gameType, ', '.join(list_game_category))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider content comm list')
