...
from O2oUtility import AdminWebPromoTypeListFrame
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer promo type list frame m'
        self.CaseNote= ''
        self.TestCaseID= '112'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_promo_type_list_frame', 'API.bankerId', '')
        ReplaceIniValue('admin_webPlayer_promo_type_list_frame', 'API.page', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo type list frame m'+'\n')
        list_result = []
        
        bankerId = IniValue('setting', 'API.bankerId')
        promoTypeName = IniValue('setting', 'API.promoTypeName')
        ReplaceIniValue('admin_webPlayer_promo_type_list_frame', 'API.bankerId', bankerId)
        ReplaceIniValue('admin_webPlayer_promo_type_list_frame', 'API.promoType', '1')
        ReplaceIniValue('admin_webPlayer_promo_type_list_frame', 'API.page', '')
        self.res_log, result, promoTypeId = AdminWebPromoTypeListFrame(self.res_log, 'Boss', promoTypeName, 'Mobile')
        
        promoTypeName2 = IniValue('setting', 'API.promoTypeName2')
        ReplaceIniValue('admin_webPlayer_promo_type_list_frame', 'API.promoType', '2')
        self.res_log, result2, promoTypeId2 = AdminWebPromoTypeListFrame(self.res_log, 'Boss', promoTypeName2, 'Mobile')
        
        promoTypeName3 = IniValue('setting', 'API.promoTypeName3')
        ReplaceIniValue('admin_webPlayer_promo_type_list_frame', 'API.promoType', '3')
        self.res_log, result3, promoTypeId3 = AdminWebPromoTypeListFrame(self.res_log, 'Boss', promoTypeName3, 'Mobile')
        
        promoTypeName4 = IniValue('setting', 'API.promoTypeName4')
        ReplaceIniValue('admin_webPlayer_promo_type_list_frame', 'API.promoType', '4')
        self.res_log, result4, promoTypeId4 = AdminWebPromoTypeListFrame(self.res_log, 'Boss', promoTypeName4, 'Mobile')
        
        promoTypeName5 = IniValue('setting', 'API.promoTypeName5')
        ReplaceIniValue('admin_webPlayer_promo_type_list_frame', 'API.promoType', '5')
        self.res_log, result5, promoTypeId5 = AdminWebPromoTypeListFrame(self.res_log, 'Boss', promoTypeName5, 'Mobile')
        
        if(result == True):
            ReplaceIniValue('setting', 'API.promoTypeId', promoTypeId)
            list_result.append('PASS')
        else:
            list_result.append('FAIL')
        
        if(result2 == True):
            ReplaceIniValue('setting', 'API.promoTypeId2', promoTypeId2)
            list_result.append('PASS')
        else:
            list_result.append('FAIL')
        
        if(result3 == True):
            ReplaceIniValue('setting', 'API.promoTypeId3', promoTypeId3)
            list_result.append('PASS')
        else:
            list_result.append('FAIL')
        
        if(result4 == True):
            ReplaceIniValue('setting', 'API.promoTypeId4', promoTypeId4)
            list_result.append('PASS')
        else:
            list_result.append('FAIL')
        
        if(result5 == True):
            ReplaceIniValue('setting', 'API.promoTypeId5', promoTypeId5)
            list_result.append('PASS')
        else:
            list_result.append('FAIL')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo type list frame m')
