...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider payOption edit get quick layer frame'
        self.CaseNote= ''
        self.TestCaseID= '145'
    ...
    def run(self, copy_result):
        ...
        result_list2 = []
        result_list3 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption edit get quick layer frame'+'\n')
        ...
        try:
            ...
            ReplaceIniValue('admin_provider_payOption_edit_get_quick_layer_frame', 'API.webId', webId)
            ReplaceIniValue('admin_provider_payOption_edit_get_quick_layer_frame', 'API.bankerId', partnerId)
            ReplaceIniValue('admin_provider_payOption_edit_get_quick_layer_frame', 'API.api_path', 'api/provider/payOption/edit/get/quick/layer/frame')
                ...
                for selected_value in selected_values:
                    if(selected_value.get('selected') == ''):
                        if(str(selected_value.string).replace('\n','').replace(' ','') == '请选择支付组合'):
                            ...
                            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success, layer and website are not linked together.')
                        else:
                            result_list2.append('FAIL')
                            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail, layer and website are linked together.')
                    elif(selected_value.get('value') == payOptionId):
                        if(str(selected_value.string).replace('\n','').replace(' ','') == payOptionName):
                            result_list3.append('PASS')
                            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get payOptionId and payOptionName.')
                        else:
                            result_list3.append('FAIL')
                            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get payOptionId and payOptionName.')
                    else:
                        result_list2.append('FAIL')
                        result_list3.append('FAIL')
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_provider_payOption_edit_get_quick_layer_frame... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption edit get quick layer frame')
