...
from O2oUtility import AdminWebProviderEdit
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer web provider edit'
        self.CaseNote= ''
        self.TestCaseID= '634'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_web_provider_edit', 'API.providerId', '')
        ReplaceIniValue('admin_webPlayer_web_provider_edit', 'API.webId', '')
        ReplaceIniValue('admin_webPlayer_web_provider_edit', 'API.status', '')
        ReplaceIniValue('admin_webPlayer_web_provider_edit', 'API.offGames', '')
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer web provider edit'+'\n')
        
        provider_ids = IniValue('setting', 'API.provider_ids')
        provider_id_list = provider_ids.split(', ')
        webId = IniValue('setting', 'API.webId')
        ReplaceIniValue('admin_webPlayer_web_provider_edit', 'API.webId', webId)
        ReplaceIniValue('admin_webPlayer_web_provider_edit', 'API.status', '1')
        ReplaceIniValue('admin_webPlayer_web_provider_edit', 'API.offGames', '')
        for provider_id in provider_id_list:
            ReplaceIniValue('admin_webPlayer_web_provider_edit', 'API.providerId', provider_id)
            
            self.res_log, result = AdminWebProviderEdit(self.res_log, 'Boss')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer web provider edit')
