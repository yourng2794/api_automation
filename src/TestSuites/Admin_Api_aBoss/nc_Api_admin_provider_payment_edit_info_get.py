...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider payment edit info get'
        self.CaseNote= ''
        self.TestCaseID= '135'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_provider_payment_edit_info_get', 'API.paymentId', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payment edit info get'+'\n')
        ...
        self.res_log, result, pay_type_id, payurl, payenable = AdminProviderPaymentEditInfoGet(self.res_log, 'Boss', paymentName, paymentId, payTypeName)
        
        if(result == True):
            ReplaceIniValue('setting', 'API.payTypeId', pay_type_id)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payment edit info get')
