...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer transfer'
        self.CaseNote= ''
        self.TestCaseID= '102'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer transfer'+'\n')
        ...
        print_tab = '\n\t\t'
        # Part 1: Create new Player
        try:
            partnerid = IniValue('setting', 'API.partnerid')
            transferVipName = IniValue('setting', 'API.transferVipName')
            ReplaceIniValue('admin_webPlayer_vip_create', 'API.bankerId', partnerid)
            ReplaceIniValue('admin_webPlayer_vip_create', 'API.vipname', transferVipName)
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_vip_create', True, False)
            ...
            self.res_log = WriteDebugLog(self.res_log, print_tab+'---')
        # Part 2: Get new Player ID
        try:
            ReplaceIniValue('admin_webPlayer_vip_list_frame', 'API.bankerId', partnerid)
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_vip_list_frame', False, False)
            WriteLog(print_tab+'Response data: '+ response_data)
            if(return_value):
                result_list.append('PASS')
                self.res_log = WriteDebugLog(self.res_log, print_tab+'- API success (status and response are correct).')
                ...
                soup_transferVipIds = soup.find_all("div")
                for soup_transferVipId in soup_transferVipIds:
                    if(str(soup_transferVipId.string).replace('\n','').replace(' ','') == transferVipName):
                        self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get transferVipName: '+transferVipName)
                        transferVipId = soup_transferVipId.get('id')
                        ReplaceIniValue('setting', 'API.transferVipId', transferVipId)
                        self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get transferVipId: '+transferVipId)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_vip_list_frame... %s' % str(e))
            
        finally:
            self.res_log = WriteDebugLog(self.res_log, print_tab+'---')
        
        # Part 3: Create new Player
        
        # Part 4: Get new Player ID
        
        # Part 5: Move new Player to Exist VIP
        
        # Part 6: Transfer new player to new VIP
        try:
            webId = IniValue('setting', 'API.webId')
            playerId = IniValue('setting', 'API.playerId')
            outVipId = IniValue('setting', 'API.transfervipid')
            ReplaceIniValue('admin_webPlayer_transfer', 'API.webId', webId)
            ReplaceIniValue('admin_webPlayer_transfer', 'API.playerId[]', playerId)
            ReplaceIniValue('admin_webPlayer_transfer', 'API.outVipId', outVipId)
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_transfer', True, False)
            WriteLog(print_tab+'Response data: '+ response_data)
            if(return_value):
                result_list.append('PASS')
                self.res_log = WriteDebugLog(self.res_log, print_tab+'- API success (status and response are correct).')
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_transfer... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer transfer')
