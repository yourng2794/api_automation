...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST notification adm group get'
        self.CaseNote= ''
        self.TestCaseID= '173'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_notification_adm_group_get', 'API.id', '')
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST notification adm group get'+'\n')
        
        print_tab = '\n\t\t'
        try:
            admin_boss_id = IniValue('setting', 'TestData.admin_boss_id')
            adm_group_title = IniValue('setting', 'API.adm_group_title')
            adm_group_msg = IniValue('setting', 'API.adm_group_msg')
            adm_group_id = IniValue('setting', 'API.adm_group_id')
            webId = IniValue('setting', 'API.webId')
            ReplaceIniValue('admin_notification_adm_group_get', 'API.id', adm_group_id)
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_notification_adm_group_get', False, False)
            ...
            if(return_value):
                ...
                if(str(json_response.get('0').get('id')) == adm_group_id):
                    result_list.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get adm_group_id: '+adm_group_id)
                else:
                    result_list.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get adm_group_id: '+str(json_response.get('0').get('id'))+' != '+adm_group_id)
                ...
                if(json_response.get('0').get('title') == adm_group_title):
                    result_list.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get adm_group_title: '+adm_group_title)
                else:
                    result_list.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get adm_group_title: '+json_response.get('0').get('title')+' != '+adm_group_title)
                ...
                if(json_response.get('content') == adm_group_msg):
                    result_list.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get adm_group_msg: '+adm_group_msg)
                else:
                    result_list.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get adm_group_msg: '+json_response.get('content')+' != '+adm_group_msg)
            else:
                result_list.append('FAIL')
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_notification_adm_group_get... %s' % str(e))
        ...
        if('FAIL' in result_list or not result_list):
            ...
            ReplaceIniValue('setting', 'API.adm_group_modifytime', modifytime)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST notification adm group get')
