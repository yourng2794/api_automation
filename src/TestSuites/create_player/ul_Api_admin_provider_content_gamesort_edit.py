...
from O2oUtility import AdminProviderContentGamesortList
from O2oUtility import AdminProviderContentGamesortEdit
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider content gamesort edit'
        self.CaseNote= ''
        self.TestCaseID= '173'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_provider_content_gamesort_list', 'API.webid', '')
        ReplaceIniValue('admin_provider_content_gamesort_list', 'API.lang', '')
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider content gamesort edit'+'\n')
        
        providerIds = IniValue('setting', 'TestData.providergames')
        webId = IniValue('setting', 'API.webId')
        language = IniValue('setting', 'TestData.language')
        ReplaceIniValue('admin_provider_content_gamesort_list', 'API.webid', webId)
        ReplaceIniValue('admin_provider_content_gamesort_list', 'API.lang', language)
        
        list_providerId = providerIds.split(', ')
        
        for each_providerId in list_providerId:
#             if(not ('LIVED' in each_providerId) or not ('SPORT' in each_providerId) or not ('ESPORT' in each_providerId)):
#                 ReplaceIniValue('admin_provider_content_gamesort_list', 'API.providerid', each_providerId)
#                 self.res_log, result, games_in_provider = AdminProviderContentGamesortList(self.res_log, 'Boss', webId, each_providerId, 'edit')
#                 result_list.append(result)
#                 ReplaceIniValue('setting', 'GameType.suggest_h_r_n_a_'+each_providerId, games_in_provider)
#                 self.res_log, result2 = AdminProviderContentGamesortEdit(self.res_log, 'Boss')
#                 result_list.append(result2)
#             else:
#                 self.res_log = WriteDebugLog(self.res_log, 'No support: '+each_providerId)

            if('SLOT' in each_providerId):
                ReplaceIniValue('admin_provider_content_gamesort_list', 'API.providerid', each_providerId)
                self.res_log, result, games_in_provider = AdminProviderContentGamesortList(self.res_log, 'Boss', webId, each_providerId, 'edit')
                result_list.append(result)
                ReplaceIniValue('setting', 'GameType.suggest_h_r_n_a_'+each_providerId, games_in_provider)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider content gamesort edit')
