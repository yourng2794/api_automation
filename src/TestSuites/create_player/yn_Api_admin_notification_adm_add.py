...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST notification adm add'
        self.CaseNote= ''
        self.TestCaseID= '173'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_notification_adm_add', 'API.webid', '')
        ReplaceIniValue('admin_notification_adm_add', 'API.bankerid', '')
        ReplaceIniValue('admin_notification_adm_add', 'API.targetids', '')
        ReplaceIniValue('admin_notification_adm_add', 'API.content', '')
        ReplaceIniValue('admin_notification_adm_add', 'API.title', '')
        ReplaceIniValue('admin_notification_adm_add', 'API.level', '')
        ReplaceIniValue('admin_notification_adm_add', 'API.type', '')
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST notification adm add'+'\n')
        ...
        try:
            ...
            ReplaceIniValue('admin_notification_adm_add', 'API.webid', webId)
            ReplaceIniValue('admin_notification_adm_add', 'API.bankerid', bankerId)
            ReplaceIniValue('admin_notification_adm_add', 'API.targetids', bc_acc)
#             ReplaceIniValue('admin_notification_adm_add', 'API.targetids', list_playeracc)
            ReplaceIniValue('admin_notification_adm_add', 'API.content', adm_msg)
            ReplaceIniValue('admin_notification_adm_add', 'API.title', adm_title)
            ReplaceIniValue('admin_notification_adm_add', 'API.level', '')
            ReplaceIniValue('admin_notification_adm_add', 'API.type', '0') # 會員:0 代理:1 末端代理:2
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_notification_adm_add', True, False)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_notification_adm_add... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST notification adm add')
