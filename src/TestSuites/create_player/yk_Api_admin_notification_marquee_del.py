...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST notification marquee del'
        self.CaseNote= ''
        self.TestCaseID= '173'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_notification_marquee_del', 'API.id', '')
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST notification marquee del'+'\n')
        
        print_tab = '\n\t\t'
        try:
            marquee_id = IniValue('setting', 'API.marquee_id')
            ReplaceIniValue('admin_notification_marquee_del', 'API.id', marquee_id)
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_notification_marquee_del', True, False)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_notification_marquee_del... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST notification marquee del')
