...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST notification adm group list frame'
        self.CaseNote= ''
        self.TestCaseID= '173'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_notification_adm_group_list_frame', 'API.webid', '')
        ReplaceIniValue('admin_notification_adm_group_list_frame', 'API.bankerid', '')
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST notification adm group list frame'+'\n')
        
        print_tab = '\n\t\t'
        try:
            adm_group_title = IniValue('setting', 'API.adm_group_title').replace(' ','')
            webId = IniValue('setting', 'API.webId')
            bankerId = IniValue('setting', 'API.bankerId')
            ReplaceIniValue('admin_notification_adm_group_list_frame', 'API.webid', webId)
            ReplaceIniValue('admin_notification_adm_group_list_frame', 'API.bankerid', bankerId)
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_notification_adm_group_list_frame', False, False)
            ...
            if(return_value):
                ...
                for soup_li in soup_lis:
                    ...
                    for soup_div in soup_divs:
                        if(str(soup_div.text).replace('\n','').replace(' ','') == adm_group_title):
                            adm_group_id = soup_li.find('a', {"href":"web_message_view.html"}).get('id')
                            ReplaceIniValue('setting', 'API.adm_group_id', adm_group_id)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_notification_adm_group_list_frame... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST notification adm group list frame')
