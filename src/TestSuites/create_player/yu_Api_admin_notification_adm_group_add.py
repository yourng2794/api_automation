...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST notification adm group dd'
        self.CaseNote= ''
        self.TestCaseID= '173'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_notification_adm_group_add', 'API.webid', '')
        ReplaceIniValue('admin_notification_adm_group_add', 'API.bankerid', '')
        ReplaceIniValue('admin_notification_adm_group_add', 'API.targetids', '')
        ReplaceIniValue('admin_notification_adm_group_add', 'API.content', '')
        ReplaceIniValue('admin_notification_adm_group_add', 'API.title', '')
        ReplaceIniValue('admin_notification_adm_group_add', 'API.level', '')
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST notification adm group dd'+'\n')
...
#             vipName = IniValue('setting', 'API.vipName')+number
            vipName = IniValue('setting', 'API.vipName')
            webId = IniValue('setting', 'API.webId')
            bankerId = IniValue('setting', 'API.bankerId')
            ReplaceIniValue('admin_notification_adm_group_add', 'API.webid', webId)
            ReplaceIniValue('admin_notification_adm_group_add', 'API.bankerid', bankerId)
            ReplaceIniValue('admin_notification_adm_group_add', 'API.targetids', '')
            ReplaceIniValue('admin_notification_adm_group_add', 'API.content', adm_group_msg)
            ReplaceIniValue('admin_notification_adm_group_add', 'API.title', adm_group_title)
            ReplaceIniValue('admin_notification_adm_group_add', 'API.level', vipName)
            ReplaceIniValue('admin_notification_adm_group_add', 'API.type', '0')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_notification_adm_group_add', True, False)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_notification_adm_group_add... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST notification adm group dd')
