...
from O2oUtility import AdminProviderContentHotEdit
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider content hot edit'
        self.CaseNote= ''
        self.TestCaseID= '173'
    ...
    def run(self, copy_result):
        ...
        ReplaceIniValue('setting', 'GameType.bottom_hot', ', '.join(list_gamename))
        
        json_dict['data'] = list_gamerule
        with open(api_json, 'w') as outfile:
            json.dump(json_dict, outfile, indent=4)
        self.res_log, result2 = AdminProviderContentHotEdit(self.res_log, 'Boss')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider content hot edit')
