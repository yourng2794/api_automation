...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer player list frame'
        self.CaseNote= ''
        self.TestCaseID= '174'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_player_list_frame', 'API.webId', '')
        ReplaceIniValue('admin_webPlayer_player_list_frame', 'API.vipId', '')
        ReplaceIniValue('admin_webPlayer_player_list_frame', 'API.playerAcc', '')
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer player list frame'+'\n')
                ...
                for soup_li in soup_lis:
                    ...
                    for soup_div in soup_divs:
                        if(str(soup_div.string).replace('\n','').replace(' ','') == playerAcc):
                            ...
                            ReplaceIniValue('setting', 'API.playerid', playerId.get('id'))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer player list frame')
