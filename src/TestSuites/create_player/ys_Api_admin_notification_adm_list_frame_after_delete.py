...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST notification adm list frame after delete'
        self.CaseNote= ''
        self.TestCaseID= '173'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_notification_adm_list_frame', 'API.webid', '')
        ReplaceIniValue('admin_notification_adm_list_frame', 'API.bankerid', '')
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST notification adm list frame after delete'+'\n')
        ...
        try:
            ...
            ReplaceIniValue('admin_notification_adm_list_frame', 'API.webid', webId)
            ReplaceIniValue('admin_notification_adm_list_frame', 'API.bankerid', bankerId)
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_notification_adm_list_frame', False, False)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_notification_adm_list_frame_after_delete... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST notification adm list frame after delete')
