...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST notification adm group list frame after delete'
        self.CaseNote= ''
        self.TestCaseID= '173'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST notification adm group list frame after delete'+'\n')
        ...
        try:
                ...
                for soup_li in soup_lis:
                    ...
                    for soup_div in soup_divs:
                        if(str(soup_div.text).replace('\n','').replace(' ','') == adm_title):
                            adm_id = soup_li.find('a', {"href":"web_message_view.html"}).get('id')
                            ReplaceIniValue('setting', 'API.adm_id', adm_id)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_notification_adm_group_list_frame_after_delete... %s' % str(e))
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST notification adm group list frame after delete')
