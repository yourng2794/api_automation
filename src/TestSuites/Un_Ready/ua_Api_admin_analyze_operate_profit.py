...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST analyze operate profit'
        self.CaseNote= ''
        self.TestCaseID= '207'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST analyze operate profit'+'\n')
        print_tab = '\n\t\t'
        try:
            bankerId = IniValue('setting', 'API.partnerid')
            webId = IniValue('setting', 'API.webid')
            webName = IniValue('setting', 'API.webname')
            ReplaceIniValue('admin_analyze_operate_profit', 'API.webId', webId)
            ReplaceIniValue('admin_analyze_operate_profit', 'API.bankerId', bankerId)
            ReplaceIniValue('admin_analyze_operate_profit', 'API.currency', 'CNY')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_analyze_operate_profit', False, False)
            ...
            if(return_value):
                ...
                if(json_response['web_site'][0] == webName):
                    result_list.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get web_site: '+webName)
                else:
                    result_list.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get web_site: '+json_response['web_site'][0]+' != '+webName)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_analyze_operate_profit... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST analyze operate profit')
