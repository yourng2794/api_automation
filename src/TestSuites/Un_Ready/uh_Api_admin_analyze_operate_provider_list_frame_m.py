...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST analyze operate provider list frame m'
        self.CaseNote= ''
        self.TestCaseID= '207'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST analyze operate provider list frame m'+'\n')
        print_tab = '\n\t\t'
        try:
            bankerId = IniValue('setting', 'API.partnerid')
            webId = IniValue('setting', 'API.webid')
            ReplaceIniValue('admin_analyze_operate_provider_list_frame', 'API.webId', webId)
            ReplaceIniValue('admin_analyze_operate_provider_list_frame', 'API.bankerId', bankerId)
            ReplaceIniValue('admin_analyze_operate_provider_list_frame', 'API.currency', 'CNY')
            ReplaceIniValue('admin_analyze_operate_provider_list_frame', 'API.subType', 'profit')
            ReplaceIniValue('admin_analyze_operate_provider_list_frame', 'API.api_path', 'api/analyze/operate/provider/list/frame/m')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_analyze_operate_provider_list_frame', True, False)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_analyze_operate_provider_list_frame_m... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST analyze operate provider list frame m')
