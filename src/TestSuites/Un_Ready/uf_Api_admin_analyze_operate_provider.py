...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST analyze operate provider'
        self.CaseNote= ''
        self.TestCaseID= '207'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST analyze operate provider'+'\n')
        print_tab = '\n\t\t'
        try:
            bankerId = IniValue('setting', 'API.partnerid')
            webId = IniValue('setting', 'API.webid')
            webName = IniValue('setting', 'API.webname')
            ReplaceIniValue('admin_analyze_operate_provider', 'API.webId', webId)
            ReplaceIniValue('admin_analyze_operate_provider', 'API.bankerId', bankerId)
            ReplaceIniValue('admin_analyze_operate_provider', 'API.currency', 'CNY')
            ReplaceIniValue('admin_analyze_operate_provider', 'API.subType', 'profit')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_analyze_operate_provider', False, False)
            ...
            if(return_value):
                ...
                if(json_response['unit'] == '万元'):
                    result_list.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get unit: 万元')
                else:
                    result_list.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get unit: '+json_response['unit']+' != 万元')
                
                if(json_response['web_name'] == webName):
                    result_list.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get web_name: '+webName)
                else:
                    result_list.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get web_name: '+json_response['web_name']+' != '+webName)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_analyze_operate_provider_m... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST analyze operate provider')
