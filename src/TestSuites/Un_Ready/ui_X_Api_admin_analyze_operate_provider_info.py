...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST analyze operate provider info'
        self.CaseNote= ''
        self.TestCaseID= '207'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST analyze operate provider info'+'\n')
        print_tab = '\n\t\t'
        try:
            ...
            ReplaceIniValue('admin_analyze_operate_provider_info', 'API.webId', webId)
            ReplaceIniValue('admin_analyze_operate_provider_info', 'API.bankerId', bankerId)
            ReplaceIniValue('admin_analyze_operate_provider_info', 'API.currency', 'CNY')
            ReplaceIniValue('admin_analyze_operate_provider_info', 'API.setTime', 'd')
            ReplaceIniValue('admin_analyze_operate_provider_info', 'API.startTime', str(datetime.today()).split(' ')[0])
            ReplaceIniValue('admin_analyze_operate_provider_info', 'API.providerId', providerId)
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_analyze_operate_provider_info', True, False)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_analyze_operate_provider_info... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST analyze operate provider info')
