...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST quickSearch organization frame m'
        self.CaseNote= ''
        self.TestCaseID= '207'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST quickSearch organization frame m'+'\n')
        print_tab = '\n\t\t'
        try:
            bankerId = IniValue('setting', 'API.partnerid')
            ReplaceIniValue('admin_quickSearch_organization_frame', 'API.status', 'all')
            ReplaceIniValue('admin_quickSearch_organization_frame', 'API.bankerId', bankerId)
            ReplaceIniValue('admin_quickSearch_organization_frame', 'API.api_path', 'api/quickSearch/organization/frame/m')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_quickSearch_organization_frame', True, False)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_quickSearch_organization_frame_m... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST quickSearch organization frame m')
