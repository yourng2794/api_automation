...
from O2oUtility import AdminVipCreate
from O2oUtility import AdminVipListFrame
from O2oUtility import AdminVipDelete
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer vip delete'
        self.CaseNote= ''
        self.TestCaseID= '385'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_vip_delete', 'API.WebMasterId', '')
        ReplaceIniValue('admin_webPlayer_vip_delete', 'API.vipId', '')
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer vip delete'+'\n')
        
        # Part 1: Create VIP.
        partnerid = IniValue('setting', 'API.partnerid')
        vipname = 'DeleteVIP'
        ReplaceIniValue('admin_webPlayer_vip_create', 'API.WebMasterId', partnerid)
        ReplaceIniValue('admin_webPlayer_vip_create', 'API.vipname', vipname)
        ReplaceIniValue('admin_webPlayer_vip_create', 'API.deposittimes', '10')
        ReplaceIniValue('admin_webPlayer_vip_create', 'API.deposittotal', '')
        ReplaceIniValue('admin_webPlayer_vip_create', 'API.maxdepositamount', '2000')
        ReplaceIniValue('admin_webPlayer_vip_create', 'API.maxwithdrawalamount', '1000')
        ReplaceIniValue('admin_webPlayer_vip_create', 'API.withdrawaltimes', '10')
        ReplaceIniValue('admin_webPlayer_vip_create', 'API.withdrawaltotal', '100')
         
        self.res_log, result = AdminVipCreate(self.res_log, 'WebMaster')
        ...
        # Part 2: Get VIP ID from web.
        ReplaceIniValue('admin_webPlayer_vip_list_frame', 'API.WebMasterId', partnerid)
        
        self.res_log, result, vipId = AdminVipListFrame(self.res_log, 'WebMaster', vipname, 'Web')
        if(result == True):
            result_list.append('PASS')
        else:
            result_list.append('FAIL')
        
        # Part 3: Get VIP ID from mobile web.
        self.res_log, result, vipId2 = AdminVipListFrame(self.res_log, 'WebMaster', vipname, 'Mobile')
        ...
        # Part 4: Delete VIP.
        ReplaceIniValue('admin_webPlayer_vip_delete', 'API.WebMasterId', partnerid)
        ReplaceIniValue('admin_webPlayer_vip_delete', 'API.vipId', vipId)
        self.res_log, result = AdminVipDelete(self.res_log, 'WebMaster')
        ...
        # Part 5: Check VIPName doesn't exist. 
#         ReplaceIniValue('admin_webPlayer_vip_list_frame', 'API.WebMasterId', partnerid)
#         self.res_log, result, vipId3 = AdminVipListFrame(self.res_log, 'WebMaster', vipname, 'Web')
#         
#         if(result == True):
#             result_list.append('PASS')
#         else:
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer vip delete')
