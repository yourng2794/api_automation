...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer promo commission list frame'
        self.CaseNote= ''
        self.TestCaseID= '623'
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo commission list frame'+'\n')
        ...
        ReplaceIniValue('admin_webPlayer_promo_commission_list_frame', 'API.webId', 'all')
        ReplaceIniValue('admin_webPlayer_promo_commission_list_frame', 'API.vipId', 'all')
        ReplaceIniValue('admin_webPlayer_promo_commission_list_frame', 'API.page', '')
        ReplaceIniValue('admin_webPlayer_promo_commission_list_frame', 'API.webCommissionName', webCommissionName)

        self.res_log, result = AdminWebPromoCommissionListFrame(self.res_log, 'WebMaster', webCommissionName, 'Web')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo commission list frame')
