...
from O2oUtility import AdminProviderPayOptionBankGet
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider payOption bank get'
        self.CaseNote= ''
        self.TestCaseID= '151'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption bank get'+'\n')
        
        payoptionId = IniValue('setting', 'API.payoptionId')
        currency = IniValue('setting', 'API.currency')
        ReplaceIniValue('admin_provider_payOption_bank_get', 'API.payoptionId', payoptionId)
        ReplaceIniValue('admin_provider_payOption_bank_get', 'API.currency', currency)
        ReplaceIniValue('admin_provider_payOption_bank_get', 'API.payOption', '1')
        bankName = IniValue('setting', 'API.bankname')
        bankacc = IniValue('setting', 'API.bankacc')
        
        self.res_log, result = AdminProviderPayOptionBankGet(self.res_log, 'WebMaster', bankName+'-'+bankacc)
        ...
        ReplaceIniValue('admin_provider_payOption_bank_get', 'API.payOption', '2')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption bank get')
