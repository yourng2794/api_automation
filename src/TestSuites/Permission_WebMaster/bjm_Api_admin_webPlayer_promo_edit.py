...
from O2oUtility import AdminWebPromoEdit
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer promo edit'
        self.CaseNote= ''
        self.TestCaseID= '115'
...
#         ReplaceIniValue('admin_webPlayer_promo_edit', 'API.promoName', '')
#         ReplaceIniValue('admin_webPlayer_promo_edit', 'API.promoId', '')
#         ReplaceIniValue('admin_webPlayer_promo_edit', 'API.startTime', '')
#         ReplaceIniValue('admin_webPlayer_promo_edit', 'API.endTime', '')
#         ReplaceIniValue('admin_webPlayer_promo_edit', 'API.condition', '')
#         ReplaceIniValue('admin_webPlayer_promo_edit', 'API.reward', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo edit'+'\n')

        now_time= datetime.now()
        end_time = now_time+ timedelta(hours=24)
        promoName = IniValue('setting', 'API.promoName')
        promoId = IniValue('setting', 'API.promoId')
        condition = IniValue('admin_webPlayer_promo_create', 'API.condition')
        reward = IniValue('admin_webPlayer_promo_create', 'API.reward')
        ReplaceIniValue('admin_webPlayer_promo_edit', 'API.promoName', promoName)
        ReplaceIniValue('admin_webPlayer_promo_edit', 'API.promoId', promoId)
        ReplaceIniValue('admin_webPlayer_promo_edit', 'API.condition', condition)
        ReplaceIniValue('admin_webPlayer_promo_edit', 'API.reward', reward)
#         ReplaceIniValue('admin_webPlayer_promo_edit', 'API.startTime', startTime)
#         ReplaceIniValue('admin_webPlayer_promo_edit', 'API.endTime', str(end_time.strftime("%Y-%m-%d %H:%M:%S")))
        time = IniValue('admin_webPlayer_promo_create', 'API.time')
        newtime = str(time.split(' ~ ')[0])+' ~ '+str(end_time.strftime("%Y-%m-%d %H:%M:%S"))
        ReplaceIniValue('admin_webPlayer_promo_edit', 'API.time', newtime)
        
        self.res_log, result = AdminWebPromoEdit(self.res_log, 'WebMaster')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo edit')
