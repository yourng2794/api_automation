...
from O2oUtility import AdminWebPromoCommissionTypeListFrame
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer promo commission type list frame m'
        self.CaseNote= ''
        self.TestCaseID= '619'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_promo_commission_type_list_frame', 'API.bankerId', '')
        ReplaceIniValue('admin_webPlayer_promo_commission_type_list_frame', 'API.page', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo commission type list frame m'+'\n')
        
        partnerid = IniValue('setting', 'API.partnerid')
        commissionTypeName = IniValue('setting', 'API.commissionTypeName')
        ReplaceIniValue('admin_webPlayer_promo_commission_type_list_frame', 'API.bankerId', partnerid)
        ReplaceIniValue('admin_webPlayer_promo_commission_type_list_frame', 'API.page', '')
        
        self.res_log, result, commissionTypeId = AdminWebPromoCommissionTypeListFrame(self.res_log, 'WebMaster', commissionTypeName, 'Mobile')
        
        if(result == True):
            ReplaceIniValue('setting', 'API.commissionTypeId', commissionTypeId)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo commission type list frame m')
