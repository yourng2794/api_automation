...
#         ReplaceIniValue('admin_webPlayer_promo_type_create', 'API.reward', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo type create'+'\n')
        ...
        ReplaceIniValue('admin_webPlayer_promo_type_create', 'API.bankerId', partnerId)
        
        self.res_log, result = AdminWebPromoTypeCreate(self.res_log, 'WebMaster')
