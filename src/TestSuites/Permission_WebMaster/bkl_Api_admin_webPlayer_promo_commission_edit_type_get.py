...
from O2oUtility import AdminWebPromoCommissionEditTypeGet
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer promo commission edit type get'
        self.CaseNote= ''
        self.TestCaseID= '628'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_promo_commission_edit_type_get', 'API.webId', '')
        ReplaceIniValue('admin_webPlayer_promo_commission_edit_type_get', 'API.vipId', '')
        ReplaceIniValue('admin_webPlayer_promo_commission_edit_type_get', 'API.type', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo commission edit type get'+'\n')
        
#         list_type = ['slot', 'sport', 'lived', 'lottery', 'fish', 'poker','esport']
        list_type = ['slot', 'sport', 'lived', 'lottery', 'fish', 'poker']
        webId = IniValue('setting', 'API.webId')
        vipId = IniValue('setting', 'API.vipId')
        ReplaceIniValue('admin_webPlayer_promo_commission_edit_type_get', 'API.webId', webId)
        ReplaceIniValue('admin_webPlayer_promo_commission_edit_type_get', 'API.vipId', vipId)

        self.res_log, result = AdminWebPromoCommissionEditTypeGet(self.res_log, 'WebMaster', list_type)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo commission edit type get')
