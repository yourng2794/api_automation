...
from O2oUtility import AdminWebPromoCreateGet
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer promo create get'
        self.CaseNote= ''
        self.TestCaseID= '574'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_promo_create_get', 'API.bankerId', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo create get'+'\n')
        
        partnerid = IniValue('setting', 'API.partnerid')
        webname = IniValue('setting', 'API.webname')
        webid = IniValue('setting', 'API.webid')
        ReplaceIniValue('admin_webPlayer_promo_create_get', 'API.bankerId', partnerid)
        
        self.res_log, result= AdminWebPromoCreateGet(self.res_log, 'WebMaster', webname, webid)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo create get')
