...
from O2oUtility import AdminWebPromoProviderContentList
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer promo providerContent list'
        self.CaseNote= ''
        self.TestCaseID= '108'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_promo_providerContent_list', 'API.type', '')
...
#         list_type = ['slot', 'sport', 'lived', 'lottery', 'fish', 'poker','esport']
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo providerContent list'+'\n')
        
        self.res_log, result = AdminWebPromoProviderContentList(self.res_log, 'WebMaster', list_type)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo providerContent list')
