...
from O2oUtility import AdminVipGroupCreate
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer vip group create'
        self.CaseNote= ''
        self.TestCaseID= '92'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_vip_group_create', 'API.WebMasterId', '')
        ReplaceIniValue('admin_webPlayer_vip_group_create', 'API.vipid[]', '')
        ReplaceIniValue('admin_webPlayer_vip_group_create', 'API.vipGroupName', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer vip group create'+'\n')
        
        partnerId = IniValue('setting', 'API.partnerId')
        vipId = IniValue('setting', 'API.vipId')
        vipGroupName = IniValue('setting', 'API.vipGroupName')
        ReplaceIniValue('admin_webPlayer_vip_group_create', 'API.WebMasterId', partnerId)
        ReplaceIniValue('admin_webPlayer_vip_group_create', 'API.vipid[]', vipId)
        ReplaceIniValue('admin_webPlayer_vip_group_create', 'API.vipGroupName', vipGroupName)
        
        self.res_log, result = AdminVipGroupCreate(self.res_log, 'WebMaster')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer vip group create')
