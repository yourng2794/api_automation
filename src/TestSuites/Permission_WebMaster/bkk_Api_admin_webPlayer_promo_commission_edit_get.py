...
from O2oUtility import AdminWebPromoCommissionEditGet
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer promo commission edit get'
        self.CaseNote= ''
        self.TestCaseID= '627'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_promo_commission_edit_get', 'API.webId', '')
        ReplaceIniValue('admin_webPlayer_promo_commission_edit_get', 'API.vipId', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo commission edit get'+'\n')
        
        webId = IniValue('setting', 'API.webId')
        vipId = IniValue('setting', 'API.vipId')
        commissionName = IniValue('setting', 'API.commissionName')
        ReplaceIniValue('admin_webPlayer_promo_commission_edit_get', 'API.webId', webId)
        ReplaceIniValue('admin_webPlayer_promo_commission_edit_get', 'API.vipId', vipId)
        
        self.res_log, result = AdminWebPromoCommissionEditGet(self.res_log, 'WebMaster', commissionName)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo commission edit get')
