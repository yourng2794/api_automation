...
from O2oUtility import AdminWebPromoCommissionListFrame
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer promo commission list frame m'
        self.CaseNote= ''
        self.TestCaseID= '624'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_promo_commission_list_frame', 'API.bankerId', '')
        ReplaceIniValue('admin_webPlayer_promo_commission_list_frame', 'API.webId', '')
        ReplaceIniValue('admin_webPlayer_promo_commission_list_frame', 'API.vipId', '')
        ReplaceIniValue('admin_webPlayer_promo_commission_list_frame', 'API.page', '')
        ReplaceIniValue('admin_webPlayer_promo_commission_list_frame', 'API.webCommissionName', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo commission list frame m'+'\n')
        
        webId = IniValue('setting', 'API.webid')
        vipId = IniValue('setting', 'API.vipid')
        webCommissionName = IniValue('setting', 'API.commissionName')
        partnerId = IniValue('setting', 'API.partnerid')
        ReplaceIniValue('admin_webPlayer_promo_commission_list_frame', 'API.bankerId', partnerId)
        ReplaceIniValue('admin_webPlayer_promo_commission_list_frame', 'API.webId', webId)
        ReplaceIniValue('admin_webPlayer_promo_commission_list_frame', 'API.vipId', vipId)
        ReplaceIniValue('admin_webPlayer_promo_commission_list_frame', 'API.page', '')
        ReplaceIniValue('admin_webPlayer_promo_commission_list_frame', 'API.webCommissionName', webCommissionName)

        self.res_log, result = AdminWebPromoCommissionListFrame(self.res_log, 'WebMaster', webCommissionName, 'Mobile')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo commission list frame m')
