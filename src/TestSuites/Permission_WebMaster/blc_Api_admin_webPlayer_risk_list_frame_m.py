...
from O2oUtility import AdminWebRiskListFrame
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer risk list frame m'
        self.CaseNote= ''
        self.TestCaseID= '120'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_risk_list_frame', 'API.type', '')
        ReplaceIniValue('admin_webPlayer_risk_list_frame', 'API.ptid', '')
        ReplaceIniValue('admin_webPlayer_risk_list_frame', 'API.page', '')
        ReplaceIniValue('admin_webPlayer_risk_list_frame', 'API.ln', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer risk list frame m'+'\n')
        
        partnerid = IniValue('setting', 'API.partnerid')
        riskname = IniValue('setting', 'API.riskname')
        ReplaceIniValue('admin_webPlayer_risk_list_frame', 'API.type', 'all')
        ReplaceIniValue('admin_webPlayer_risk_list_frame', 'API.ptid', partnerid)
        ReplaceIniValue('admin_webPlayer_risk_list_frame', 'API.page', '')
        ReplaceIniValue('admin_webPlayer_risk_list_frame', 'API.ln', '')
        
        self.res_log, result, riskId = AdminWebRiskListFrame(self.res_log, 'WebMaster', riskname, 'Mobile')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer risk list frame m')
