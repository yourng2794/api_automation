...
from O2oUtility import AdminWebPromoCommissionCreateGet
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer promo commission create get'
        self.CaseNote= ''
        self.TestCaseID= '620'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_promo_commission_create_get', 'API.bankerId', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo commission create get'+'\n')
        ...
        ReplaceIniValue('admin_webPlayer_promo_commission_create_get', 'API.bankerId', partnerid)
        
        self.res_log, result = AdminWebPromoCommissionCreateGet(self.res_log, 'WebMaster', commissionTypeName, commissionTypeId)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo commission create get')
