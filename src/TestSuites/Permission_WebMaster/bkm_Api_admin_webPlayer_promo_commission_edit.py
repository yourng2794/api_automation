...
from O2oUtility import AdminWebPromoCommissionEdit
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer promo commission edit'
        self.CaseNote= ''
        self.TestCaseID= '629'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.webId', '')
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.vipId', '')
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.webCommissionName', '')
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.commissionTimes', '')
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.receiveTimesDaily', '')
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.totalBet', '')
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.promotionLimit', '')
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.maxReceiveAmount', '')
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.singleMinReceiveAmount', '')
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.fish', '')
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.lived', '')
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.lottery', '')
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.poker', '')
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.slot', '')
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.sport', '')
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.esport', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo commission edit'+'\n')
        
        webId = IniValue('setting', 'API.webid')
        vipId = IniValue('setting', 'API.vipid')
        commissionName = IniValue('setting', 'API.commissionName')
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.webId', webId)
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.vipId', vipId)
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.webCommissionName', commissionName) # 範本名稱
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.commissionTimes', '200') # 打碼倍數
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.receiveTimesDaily', '5') # 每日領取次數
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.totalBet', '400') # 有效總投注
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.promotionLimit', '500') # 優惠上限
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.maxReceiveAmount', '600') # 單日最高領取金額
        ReplaceIniValue('admin_webPlayer_promo_commission_edit', 'API.singleMinReceiveAmount', '100') # 單次最低領取金額
        
        self.res_log, result = AdminWebPromoCommissionEdit(self.res_log, 'WebMaster')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo commission edit')
