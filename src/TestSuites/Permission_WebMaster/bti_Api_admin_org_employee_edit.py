...
from O2oUtility import AdminOrgEmployeeEdit
...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST org. employee edit'
        self.CaseNote= ''
        self.TestCaseID= '204'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_org_employee_edit', 'API.userId', '')
        ReplaceIniValue('admin_org_employee_edit', 'API.userAcc', '')
        ReplaceIniValue('admin_org_employee_edit', 'API.userPwd', '')
        ReplaceIniValue('admin_org_employee_edit', 'API.depId', '')
        ReplaceIniValue('admin_org_employee_edit', 'API.titleId', '')
        ReplaceIniValue('admin_org_employee_edit', 'API.website[]', '')
        ReplaceIniValue('admin_org_employee_edit', 'API.userNote', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. employee edit'+'\n')
        
        userId = IniValue('setting', 'API.userid')
        ReplaceIniValue('admin_org_employee_edit', 'API.userId', userId)
        ReplaceIniValue('admin_org_employee_edit', 'API.userAcc', '')
        ReplaceIniValue('admin_org_employee_edit', 'API.userPwd', '')
        ReplaceIniValue('admin_org_employee_edit', 'API.depId', '')
        ReplaceIniValue('admin_org_employee_edit', 'API.titleId', '')
        ReplaceIniValue('admin_org_employee_edit', 'API.website[]', '')
        ReplaceIniValue('admin_org_employee_edit', 'API.userNote', 'After Edit')
        
        self.res_log, result = AdminOrgEmployeeEdit(self.res_log, 'WebMaster')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. employee edit')
