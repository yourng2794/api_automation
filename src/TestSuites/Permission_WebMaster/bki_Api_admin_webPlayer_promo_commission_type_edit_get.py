...
from O2oUtility import AdminWebPromoCommissionTypeEditGet
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer promo commission type edit get'
        self.CaseNote= ''
        self.TestCaseID= '625'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_webPlayer_promo_commission_type_edit_get', 'API.commissionTypeId', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo commission type edit get'+'\n')
        
        commissionTypeName = IniValue('setting', 'API.commissionTypeName')
        commissionTypeId = IniValue('setting', 'API.commissionTypeId')
        ReplaceIniValue('admin_webPlayer_promo_commission_type_edit_get', 'API.commissionTypeId', commissionTypeId)

        self.res_log, result = AdminWebPromoCommissionTypeEditGet(self.res_log, 'WebMaster', commissionTypeName, commissionTypeId)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo commission type edit get')
