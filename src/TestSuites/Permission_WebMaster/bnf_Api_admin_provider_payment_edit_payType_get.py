...
from O2oUtility import AdminProviderPaymentPayTypeGet
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider payment edit PayType get'
        self.CaseNote= ''
        self.TestCaseID= '136'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_provider_payment_edit_payType_get', 'API.paymentId', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payment edit PayType get'+'\n')
        
        paymentId = IniValue('setting', 'API.paymentId')
        payTypeId = IniValue('setting', 'API.payTypeId')
        ReplaceIniValue('admin_provider_payment_edit_payType_get', 'API.paymentId', paymentId)
        ReplaceIniValue('admin_provider_payment_edit_payType_get', 'API.payTypeId', payTypeId)
        
        self.res_log, result = AdminProviderPaymentPayTypeGet(self.res_log, 'WebMaster', payTypeId, paymentId)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payment edit PayType get')
