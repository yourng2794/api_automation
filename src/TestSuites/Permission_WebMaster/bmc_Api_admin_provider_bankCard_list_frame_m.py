...
from O2oUtility import AdminProviderBankCardListFrame
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider bankCard list frame m'
        self.CaseNote= ''
        self.TestCaseID= '130'
    ...
    def teardown(self):
        ...
        ReplaceIniValue('admin_provider_bankCard_list_frame', 'API.bankerId', '')
        ReplaceIniValue('admin_provider_bankCard_list_frame', 'API.option', '')
        ReplaceIniValue('admin_provider_bankCard_list_frame', 'API.enable', '')
        ReplaceIniValue('admin_provider_bankCard_list_frame', 'API.page', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider bankCard list frame m'+'\n')
        
        partnerid = IniValue('setting', 'API.partnerid')
        ReplaceIniValue('admin_provider_bankCard_list_frame', 'API.bankerId', partnerid)
        ReplaceIniValue('admin_provider_bankCard_list_frame', 'API.option', '0')
        ReplaceIniValue('admin_provider_bankCard_list_frame', 'API.enable', '2')
        ReplaceIniValue('admin_provider_bankCard_list_frame', 'API.page', '')
        bankName = IniValue('setting', 'API.bankname')
        
        self.res_log, result, cardId = AdminProviderBankCardListFrame(self.res_log, 'WebMaster', bankName, 'Mobile')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider bankCard list frame m')
