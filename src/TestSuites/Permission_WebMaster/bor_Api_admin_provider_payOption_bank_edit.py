...
from O2oUtility import AdminProviderPayOptionBankEdit
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider payOption bank edit'
        self.CaseNote= ''
        self.TestCaseID= '151'
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption bank edit'+'\n')
        
        payoptionId = IniValue('setting', 'API.payoptionId')
        ReplaceIniValue('admin_provider_payOption_bank_edit', 'API.payoptionId', payoptionId)
        ReplaceIniValue('admin_provider_payOption_bank_edit', 'API.withdraw[]', '')
        ReplaceIniValue('admin_provider_payOption_bank_edit', 'API.deposit[]', '')
        
        self.res_log, result = AdminProviderPayOptionBankEdit(self.res_log, 'WebMaster')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payOption bank edit')
