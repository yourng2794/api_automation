...
from O2oUtility import AdminProviderPaymentPayTypeEdit
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST provider payment payType edit'
        self.CaseNote= ''
        self.TestCaseID= '136'
...
#         ReplaceIniValue('admin_provider_payment_payType_edit', 'API.paymentId', '')
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payment payType edit'+'\n')
        
        paymentId = IniValue('setting', 'API.paymentId')
        payTypeId = IniValue('setting', 'API.payTypeId')
        ReplaceIniValue('admin_provider_payment_payType_edit', 'API.paymentId', paymentId)
        ReplaceIniValue('admin_provider_payment_payType_edit', 'API.payTypeId', payTypeId)
        ReplaceIniValue('admin_provider_payment_payType_edit', 'API.option', '3')
        ReplaceIniValue('admin_provider_payment_payType_edit', 'API.withdrawAcc', '666666')
        ReplaceIniValue('admin_provider_payment_payType_edit', 'API.withdrawKey', 'f455ff77fa023376d810a97b68e54e33')
        ReplaceIniValue('admin_provider_payment_payType_edit', 'API.withdrawRatio', '5')
        ReplaceIniValue('admin_provider_payment_payType_edit', 'API.maxWithdraw', '6666')
        ReplaceIniValue('admin_provider_payment_payType_edit', 'API.depositAcc', '888888')
        ReplaceIniValue('admin_provider_payment_payType_edit', 'API.depositKey', 'f455ff77fa023376d810a97b68e54e32')
        ReplaceIniValue('admin_provider_payment_payType_edit', 'API.depositRatio', '5')
        ReplaceIniValue('admin_provider_payment_payType_edit', 'API.maxDeposit', '88888')
        ReplaceIniValue('admin_provider_payment_payType_edit', 'API.minDeposit', '10')
        
        self.res_log, result = AdminProviderPaymentPayTypeEdit(self.res_log, 'WebMaster')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST provider payment payType edit')
