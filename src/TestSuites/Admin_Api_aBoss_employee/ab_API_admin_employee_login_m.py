...
from O2oUtility import AdminLogin
...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Portal] API POST login Employee account (mobile)'
        self.CaseNote= ''
        self.TestCaseID= '375'
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Portal] API POST login Employee account (mobile)'+'\n')
        number = IniValue('setting', 'API.number')
        acc = IniValue('setting', 'API.employeeacc1')+str(number)
        ReplaceIniValue('admin_login', 'API.acc', acc)
        ReplaceIniValue('admin_login', 'API.pwd', acc)
        
        self.res_log, result = AdminLogin(self.res_log, 'Boss', self.filename, 'Mobile')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Portal] API POST login Employee account (mobile)')
