...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[FET] API POST provider bankCard create'
        self.CaseNote= ''
        self.TestCaseID= '505'
    ...
    def run(self, copy_result):
        ...
        print_tab = '\n\t\t'
        try:
            webmaster_bankname = IniValue('setting', 'API.webmaster_bankname')
            webmaster_branchname = IniValue('setting', 'API.webmaster_branchname')
            webmaster_bankacc = IniValue('setting', 'API.webmaster_bankacc')
            web_master_name = IniValue('setting', 'API.web_master_name')
            web_master_id = IniValue('setting', 'API.web_master_id')
            currency = IniValue('setting', 'API.currency')
            ReplaceIniValue('admin_provider_bankCard_create', 'API.bankerId', web_master_id)
            ReplaceIniValue('admin_provider_bankCard_create', 'API.bankName', webmaster_bankname)
            ReplaceIniValue('admin_provider_bankCard_create', 'API.branchName', webmaster_branchname)
            ReplaceIniValue('admin_provider_bankCard_create', 'API.bankAcc', webmaster_bankacc)
            ReplaceIniValue('admin_provider_bankCard_create', 'API.accName', web_master_name)
            ReplaceIniValue('admin_provider_bankCard_create', 'API.currency', currency)
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_provider_bankCard_create', False, False)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_provider_bankCard_create... %s' % str(e))
