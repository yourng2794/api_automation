...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST partner list sub frame m'
        self.CaseNote= ''
        self.TestCaseID= '382'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner list sub frame m'+'\n')
        print_tab = '\n\t\t'
        try:
            agent_id_level4 = IniValue('setting', 'API.agent_id_level4')
            ReplaceIniValue('admin_partner_list_sub_frame', 'API.parentId', agent_id_level4)
            ReplaceIniValue('admin_partner_list_sub_frame', 'API.api_path', 'api/partner/list/sub/frame/m')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_partner_list_sub_frame', False, False)
            ...
            if(return_value):
                ...
                agent_level5 = IniValue('setting', 'API.agent_level5')
                ...
                for soup_li in soup_lis:
                    ...
                    for soup_div in soup_divs:
                        if(str(soup_div.string).replace('\n','').replace(' ','') == agent_level5):
                            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get agent_level5: '+agent_level5)
                            agent_id_level5 = soup_div.get('id')
                            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get agent_id_level5: '+agent_id_level5)
                            ReplaceIniValue('setting', 'API.agent_id_level5', agent_id_level5)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_partner_list_sub_frame_m... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner list sub frame m')
