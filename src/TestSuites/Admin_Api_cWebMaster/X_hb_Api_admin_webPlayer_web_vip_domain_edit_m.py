...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer web vip domain edit m'
        self.CaseNote= ''
        self.TestCaseID= '100'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer web vip domain edit m'+'\n')
        ...
        try:
            ...
            ReplaceIniValue('admin_webPlayer_web_vip_domain_edit', 'API.webId', webId)
            ReplaceIniValue('admin_webPlayer_web_vip_domain_edit', 'API.vipGroupId', vipGroupId)
...
#             dict_vipDataList = nested_dict()
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_web_vip_domain_edit_m... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer web vip domain edit m')
