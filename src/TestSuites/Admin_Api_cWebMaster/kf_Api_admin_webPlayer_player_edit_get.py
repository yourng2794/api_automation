...
class TestCase(BaseCase):
    ...
    def run(self, copy_result):
        ...
        try:
            ...
            ReplaceIniValue('admin_webPlayer_player_edit_get', 'API.playerId', webmaster_member_id)
            ...
            if(return_value):
                ...
                webmaster_member = IniValue('setting', 'API.webmaster_member')
                vipId = IniValue('setting', 'API.vipid')
                webId = IniValue('setting', 'API.webid')
                json_response = json.loads(response_data)
                if(json_response['acc'] == webmaster_member):
                    result_list.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get playerAcc: '+webmaster_member)
                else:
                    result_list.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get playerAcc: '+json_response['acc']+' != '+webmaster_member)
