...
class TestCase(BaseCase):
    ...
    def run(self, copy_result):
        ...
        try:
            ...
            ReplaceIniValue('admin_webPlayer_vip_group_edit_get_frame', 'API.vipGroupId', vipGroupId)
            ...
            if(return_value):
                ...
                if(soup.find("div", id=vipId).string.replace('\n','').replace(' ','') == vipName):
                    ...
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get : '+vipName)
                else:
                    result_list2.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get vipName: '+soup.find("div", id=vipId).string.replace('\n','').replace(' ','')+' != '+vipName)
