...
class TestCase(BaseCase):
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST web domain delete'+'\n')
        ...
        try:
            ...
            if(return_value3):
                ...
                webDomainId2 = soup3.find('a',attrs={'alt':'修改'}).get('id')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST web domain delete')
