...
class TestCase(BaseCase):
    ...
    def run(self, copy_result):
        ...
        try:
            ...
            ReplaceIniValue('admin_partner_info_get', 'API.ptId', agent_id_level4)
            ...
            if(return_value):
                ...
                agent_level4 = IniValue('setting', 'API.agent_level4')
                webId = IniValue('setting', 'API.webid')
                if(json_response.get('pt_acc') == agent_level4):
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get pt_acc: '+agent_level4)
                    result_list2.append('PASS')
                else:
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get pt_acc: '+json_response.get('pt_acc')+' != '+agent_level4)
                    result_list2.append('FAIL')
                
                if(json_response.get('pt_id') == agent_id_level4):
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get pt_id: '+agent_id_level4)
                    result_list2.append('PASS')
                else:
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get pt_id: '+json_response.get('pt_id')+' != '+agent_id_level4)
                    result_list2.append('FAIL')
                
                if(json_response.get('pt_website')[0].get('web_id') == webId):
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get pt_website: '+webId)
                    result_list2.append('PASS')
                else:
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get pt_website: '+json_response.get('pt_website')[0].get('web_id')+' != '+webId)
                    result_list2.append('FAIL')
