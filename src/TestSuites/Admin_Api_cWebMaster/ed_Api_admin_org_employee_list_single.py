...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST org. employee list single'
        self.CaseNote= ''
        self.TestCaseID= '563'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. employee list single'+'\n')
        print_tab = '\n\t\t'
        try:
            agent_level4_dept_employee_id = IniValue('setting', 'API.agent_level4_dept_employee_id')
            agent_level4_dept_employee = IniValue('setting', 'API.agent_level4_dept_employee')
            ...
            ReplaceIniValue('admin_org_employee_list_single', 'API.userId', agent_level4_dept_employee_id)
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_org_employee_list_single', False, False)
            ...
            if(return_value):
                ...
                if(json_response['user_acc'] == agent_level4_dept_employee):
                    result_list2.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get userAcc: '+agent_level4_dept_employee)
                else:
                    result_list2.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get userAcc')
                
                if(json_response['dp_sid'] == agent_level4_dept_title_id):
                    result_list2.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get depTitleId: '+agent_level4_dept_title_id)
                else:
                    result_list2.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get depTitleId')
                
                if(json_response['og_sid'] == agent_level4_dept_id):
                    result_list2.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get depId: '+agent_level4_dept_id)
                else:
                    result_list2.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get depId')
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_org_employee_list_single... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. employee list single')
