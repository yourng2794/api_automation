...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST partner payment get'
        self.CaseNote= ''
        self.TestCaseID= '388'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner payment get'+'\n')
        print_tab = '\n\t\t'
        try:
            ...
            ReplaceIniValue('admin_partner_payment_get', 'API.parentId', web_master_id)
            ReplaceIniValue('admin_partner_payment_get', 'API.currency', json.dumps(json_currency))
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_partner_payment_get', False, False)
            ...
            if(return_value):
                ...
                if(json_response['Error_code'] == 'S01' and json_response['Error_message'] == 'Success'):
                    result_list.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get (Error_code, Error_message): (S01, Success)')
                else:
                    result_list.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get (Error_code, Error_message): ('+json_response['Error_code']+', '+json_response['Error_message']+')')
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_partner_payment_get... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner payment get')
