...
class TestCase(BaseCase):
    ...
    def run(self, copy_result):
        ...
        try:
            ...
            vipGroupName = IniValue('setting', 'API.vipGroupName')
            transferSet = IniValue('setting', 'API.transferSet')
            setTime = IniValue('setting', 'API.setTime')
            ReplaceIniValue('admin_webPlayer_web_vip_list_frame', 'API.webId', webId)
...
#                 soup_selecteds = soup.find_all('option', attrs={"selected": ""})
                ...
                if(str(soup_selecteds[0].string).replace('\n','').replace(' ','') == vipGroupName):
                    ...
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get vipGroupName: '+vipGroupName)
                else:
                    result_list2.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get vipGroupName: '+str(soup_selecteds[0].string).replace('\n','').replace(' ','')+' != '+vipGroupName)
