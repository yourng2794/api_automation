...
from O2oUtility import AdminPartnerListFrame
...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST partner list frame'
        self.CaseNote= ''
        self.TestCaseID= '424'
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner list frame'+'\n')
        
        agent_level4 = IniValue('setting', 'API.agent_level4')
        ReplaceIniValue('admin_partner_list_frame', 'API.searchType', 'acc')
        ReplaceIniValue('admin_partner_list_frame', 'API.search', agent_level4)
        ReplaceIniValue('admin_partner_list_frame', 'API.listType', '')
        ReplaceIniValue('admin_partner_list_frame', 'API.page', '')
        self.res_log, result, agent_id_level4 = AdminPartnerListFrame(self.res_log, 'WebMaster', agent_level4, 'Web')
        
        if(result == True):
            ReplaceIniValue('setting', 'API.agent_id_level4', agent_id_level4)
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner list frame')
