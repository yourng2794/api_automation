...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer promo list frame m'
        self.CaseNote= ''
        self.TestCaseID= '485'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo list frame m'+'\n')
        ...
        try:
            ...
            ReplaceIniValue('admin_webPlayer_promo_list_frame', 'API.ptid', web_master_id)
            ReplaceIniValue('admin_webPlayer_promo_list_frame', 'API.api_path', 'api/webPlayer/promo/list/frame/m')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_promo_list_frame', False, False)
                    ...
                    for soup_li in soup_lis:
                        ...
                        for soup_div in soup_divs:
                            if(str(soup_div.string).replace('\n','').replace(' ','') == promoName):
                                self.res_log = WriteDebugLog(self.res_log, print_tab+'Success get promoName: '+promoName)
                                promoId = soup_li.find('i', id=True).get('id')
                                ReplaceIniValue('setting', 'API.promoId', promoId)
                                self.res_log = WriteDebugLog(self.res_log, print_tab+'Success get promoId: '+promoId)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_promo_list_frame_m... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo list frame m')
