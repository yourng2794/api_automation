...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST partner transfer list frame m'
        self.CaseNote= ''
        self.TestCaseID= '481'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner transfer list frame m'+'\n')
        print_tab = '\n\t\t'
        try:
            ...
            ReplaceIniValue('admin_partner_transfer_list_frame', 'API.search', agent_level4)
            ReplaceIniValue('admin_partner_transfer_list_frame', 'API.id', '')
            ReplaceIniValue('admin_partner_transfer_list_frame', 'API.api_path', 'api/partner/transfer/list/frame/m')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_partner_transfer_list_frame', False, False)
            ...
            if(return_value):
                ...
                my_attribute = soup.find('input',id=True).get('id')
                if(my_attribute == agent_id_level4):
                    result_list2.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get agent_id_level4: '+agent_id_level4)
                else:
                    result_list2.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get agent_id_level4: '+my_attribute+' != '+agent_id_level4)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_partner_transfer_list_frame_m... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner transfer list frame m')
