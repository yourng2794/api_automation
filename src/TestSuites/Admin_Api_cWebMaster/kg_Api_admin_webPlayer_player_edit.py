...
class TestCase(BaseCase):
    ...
    def run(self, copy_result):
        ...
        print_tab = '\n\t\t'
        try:
            webmaster_member_name = IniValue('setting', 'API.webmaster_member_name')
            myName = IniValue('setting', 'API.myName')
            webmaster_member_id = IniValue('setting', 'API.webmaster_member_id')
            ReplaceIniValue('admin_webPlayer_player_edit', 'API.playerPwd', webmaster_member_name)
            ReplaceIniValue('admin_webPlayer_player_edit', 'API.playerName', myName)
            ReplaceIniValue('admin_webPlayer_player_edit', 'API.playerId', webmaster_member_id)
            ReplaceIniValue('admin_webPlayer_player_edit', 'API.playerStatus', '3') # 1:啟用 2:停權 3:凍結 4:停用
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_player_edit', True, False)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_player_edit... %s' % str(e))
        ...
        try:
            ...
            if(return_value):
                ...
                if(json_response['acc'] == webmaster_member_name):
                    result_list.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get playerAcc: '+webmaster_member_name)
                else:
                    result_list.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get playerAcc: '+json_response['acc']+' != '+webmaster_member_name)
                
                if(json_response['enable'] == 3):
                    result_list.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get enable: 3(凍結)')
                else:
                    result_list.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get enable: '+json_response['enable']+' != 3(凍結)')
                self.res_log = WriteDebugLog(self.res_log, print_tab+'---')
        ...
        try:
            ...
            if(return_value):
                ...
                if(json_response['enable'] == 1):
                    result_list.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get enable: 1(啟用)')
                else:
                    result_list.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get enable: '+json_response['enable']+' != 1(啟用)')
                self.res_log = WriteDebugLog(self.res_log, print_tab+'---')
