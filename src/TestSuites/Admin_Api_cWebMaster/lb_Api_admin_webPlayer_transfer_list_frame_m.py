...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer transfer list frame m'
        self.CaseNote= ''
        self.TestCaseID= '543'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer transfer list frame m'+'\n')
        ...
        try:
            ...
            ReplaceIniValue('admin_webPlayer_transfer_list_frame', 'API.webId', webId)
            ReplaceIniValue('admin_webPlayer_transfer_list_frame', 'API.api_path', 'api/webPlayer/transfer/list/frame/m')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_transfer_list_frame', False, False)
            ...
            if(return_value):
                ...
                for soup_span in soup_spans:
                    if(str(soup_span.string).replace('\n','').replace(' ','') == webmaster_member_name):
                        ...
                        if(player_id == webmaster_member_id):
                            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get playerId: '+webmaster_member_id)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_transfer_list_frame_m... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer transfer list frame m')
