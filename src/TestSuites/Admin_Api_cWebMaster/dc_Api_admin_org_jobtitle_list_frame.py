...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST org. jobtitle list frame'
        self.CaseNote= ''
        self.TestCaseID= '555'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. jobtitle list frame'+'\n')
        print_tab = '\n\t\t'
        try:
            ...
            ReplaceIniValue('admin_org_jobtitle_list_frame', 'API.depId', agent_level4_dept_id)
            ReplaceIniValue('admin_org_jobtitle_list_frame', 'API.api_path', 'api/organization/department/jobtitle/list/frame')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_org_jobtitle_list_frame', False, False)
            ...
            if(return_value):
                ...
                for soup_li in soup_lis:
                    ...
                    for soup_div in soup_divs:
                        if(str(soup_div.string).replace('\n','').replace(' ','') == agent_level4_dept_title):
                            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get depTitle: '+agent_level4_dept_title)
                            agent_level4_dept_title_id = soup_li.find('a', attrs={'href':'organize_office_edit.html'}).get('id')
                            ReplaceIniValue('setting', 'API.agent_level4_dept_title_id', agent_level4_dept_title_id)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_org_jobtitle_list_frame... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. jobtitle list frame')
