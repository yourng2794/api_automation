...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer transfer all'
        self.CaseNote= ''
        self.TestCaseID= '545'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer transfer all'+'\n')
...
#             playerAcc = IniValue('setting', 'API.playeracc')
            ReplaceIniValue('admin_webPlayer_transfer_all', 'API.webId', webId)
#             ReplaceIniValue('admin_webPlayer_transfer_all', 'API.vipId', vipId)
            ReplaceIniValue('admin_webPlayer_transfer_all', 'API.outVipId', vipId)
#             ReplaceIniValue('admin_webPlayer_transfer_all', 'API.playerAcc', playerAcc)
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_transfer_all', True, False)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_transfer_all... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer transfer all')
