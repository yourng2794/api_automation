...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer player delete'
        self.CaseNote= ''
        self.TestCaseID= '541'
    ...
    def run(self, copy_result):
        ...
        result_list3 = []
        result_list4 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer player delete'+'\n')
        ...
        print_tab = '\n\t\t'
        # Part 1: Create test player for delete.
        try:
            playerAcc = 'DeletePlayer'
            myName = IniValue('setting', 'API.myName')
            vipId = IniValue('setting', 'API.vipid')
            webId = IniValue('setting', 'API.webid')
            ReplaceIniValue('admin_webPlayer_player_create', 'API.playerAcc', playerAcc)
            ReplaceIniValue('admin_webPlayer_player_create', 'API.playerPwd', playerAcc)
            ReplaceIniValue('admin_webPlayer_player_create', 'API.playerName', myName)
            ReplaceIniValue('admin_webPlayer_player_create', 'API.vipId', vipId)
            ReplaceIniValue('admin_webPlayer_player_create', 'API.webId', webId)
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_player_create', True, False)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_player_create... %s' % str(e))
            
        finally:
            self.res_log = WriteDebugLog(self.res_log, print_tab+'---')
        
        # Part 2: Get test player ID from web.
        try:
            ReplaceIniValue('admin_webPlayer_player_list_frame', 'API.webId', webId)
            ReplaceIniValue('admin_webPlayer_player_list_frame', 'API.playerAcc', playerAcc)
            ReplaceIniValue('admin_webPlayer_player_list_frame', 'API.api_path', 'api/webPlayer/player/list/frame')
            return_value2, response_time2, response_data2, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_player_list_frame', False, False)
            ...
            if(return_value2):
                ...
                soup = BeautifulSoup(response_data2, 'html.parser')
                ...
                for soup_li in soup_lis:
                    ...
                    for soup_div in soup_divs:
                        if(str(soup_div.string).replace('\n','').replace(' ','') == playerAcc):
                            ...
                            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get playerId: '+playerId)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_player_list_frame... %s' % str(e))
            
        finally:
            self.res_log = WriteDebugLog(self.res_log, print_tab+'---')
        
        # Part 3: Get test player ID from mobile web.
        try:
            ReplaceIniValue('admin_webPlayer_player_list_frame', 'API.webId', webId)
            ReplaceIniValue('admin_webPlayer_player_list_frame', 'API.playerAcc', playerAcc)
            ReplaceIniValue('admin_webPlayer_player_list_frame', 'API.api_path', 'api/webPlayer/player/list/frame/m')
            return_value3, response_time3, response_data3, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_player_list_frame', False, False)
            ...
            if(return_value3):
                ...
                soup_div = soup2.find("div", class_='crm-name')
                if(playerAcc in soup_div.text):
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get playerAcc2: '+playerAcc)
                    playerId2 = soup_div.find("div", class_='vip-type').get('id')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get playerId2: '+playerId2)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_player_list_frame_m... %s' % str(e))
            
        finally:
            self.res_log = WriteDebugLog(self.res_log, print_tab+'---')
        
        # Part 4: Delete player.
        try:
            ReplaceIniValue('admin_webPlayer_player_delete', 'API.playerId', playerId)
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_player_delete', True, False)
            WriteLog(print_tab+'Response data: '+ response_data)
            if(return_value):
                result_list.append('PASS')
                self.res_log = WriteDebugLog(self.res_log, print_tab+'- API success (status and response are correct).')
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_player_delete... %s' % str(e))
            
        finally:
            self.res_log = WriteDebugLog(self.res_log, print_tab+'---')
        
        # Part 5: Check PlayerName doesn't exist. 
        try:
            ReplaceIniValue('admin_webPlayer_player_list_frame', 'API.webId', webId)
            ReplaceIniValue('admin_webPlayer_player_list_frame', 'API.playerAcc', playerAcc)
            ReplaceIniValue('admin_webPlayer_player_list_frame', 'API.api_path', 'api/webPlayer/player/list/frame')
                ...
                for soup_li in soup_lis:
                    ...
                    for soup_div in soup_divs:
                        if(str(soup_div.string).replace('\n','').replace(' ','') == playerAcc):
                            ...
                            playerId = soup_li.find("a", attrs={'href':'web_mem_edit.html'}).get('id')
                            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get playerId: '+playerId)
                            result_list4.append('PASS')
                            break
                        else:
                            result_list4.append('FAIL')
                self.res_log = WriteDebugLog(self.res_log, print_tab+'---')
        ...
        WriteLog(result_list4)
        if('FAIL' in str(result_list) or not result_list):
            self.TestResult = 'FAIL'
        else:
            if('PASS' in result_list2 and 'PASS' in result_list3 and (not('PASS' in result_list4))):
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer player delete')
