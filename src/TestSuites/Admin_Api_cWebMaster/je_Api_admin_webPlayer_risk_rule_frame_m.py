...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer risk rule frame m'
        self.CaseNote= ''
        self.TestCaseID= '499'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer risk rule frame m'+'\n')
        ...
        print_tab = '\n\t\t'
        try:
            api_json = tooldir+'\\JsonFiles\\Api_Json\\admin_risk_create.json'
            web_master_id = IniValue('setting', 'API.web_master_id')
            riskid = IniValue('setting', 'API.riskid')
            ReplaceIniValue('admin_webPlayer_risk_rule_frame', 'API.ptid', web_master_id)
            ReplaceIniValue('admin_webPlayer_risk_rule_frame', 'API.riskid', riskid)
            opt_interval = CheckJsonFile(api_json, 'opt|0|interval')
            opt_count = CheckJsonFile(api_json, 'opt|0|count')
            ReplaceIniValue('admin_webPlayer_risk_rule_frame', 'API.api_path', 'api/webPlayer/risk/rule/frame/m')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_risk_rule_frame', False, False)
            ...
            if(return_value):
                ...
                if(risk_interval.get('value') == opt_interval):
                    result_list.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get interval: '+opt_interval)
                else:
                    result_list.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get interval: '+risk_interval.get('value')+' != '+opt_interval)
                
                risk_count = soup.find("input", id="count")
                if(risk_count.get('value') == opt_count):
                    result_list.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get count: '+opt_count)
                else:
                    result_list.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get interval: '+risk_count.get('value')+' != '+opt_count)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_risk_rule_frame_m... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer risk rule frame m')
