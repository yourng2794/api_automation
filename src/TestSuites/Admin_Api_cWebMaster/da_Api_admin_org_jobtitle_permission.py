...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST org. jobtitle permission'
        self.CaseNote= ''
        self.TestCaseID= '553'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. jobtitle permission'+'\n')
        print_tab = '\n\t\t'
        try:
            agent_level4_dept_id = IniValue('setting', 'API.agent_level4_dept_id')
            ReplaceIniValue('admin_org_jobtitle_permission', 'API.depId', agent_level4_dept_id)
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_org_jobtitle_permission', False, False)
            ...
            if(return_value):
                ...
                result_list2.append(JsonValueCompare(response_data, 'og', 0))
                result_list2.append(JsonValueCompare(response_data, 'op', 0))
                result_list2.append(JsonValueCompare(response_data, 'ot', 0))
                result_list2.append(JsonValueCompare(response_data, 'pd', 0))
                result_list2.append(JsonValueCompare(response_data, 'pt', 0))
                result_list2.append(JsonValueCompare(response_data, 'pyd', 0))
                result_list2.append(JsonValueCompare(response_data, 'pyf', 0))
                result_list2.append(JsonValueCompare(response_data, 'ws', 0))
                json_string = json.loads(response_data)
                json_data = json.dumps(json_string)
                json_data = ReplaceAllJsonValue(json_data, '', '1')
                ReplaceIniValue('admin_org_jobtitle_create', 'API.permission', json_data.replace(' ',''))
                ReplaceIniValue('admin_org_jobtitle_edit', 'API.permission', json_data.replace(' ',''))
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_org_jobtitle_permission... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. jobtitle permission')
