...
class TestCase(BaseCase):
    ...
    def run(self, copy_result):
        ...
        try:
            ...
            ReplaceIniValue('admin_webPlayer_risk_web_frame', 'API.ptid', web_master_id)
                ...
                for soup_vipLabelItem in soup_vipLabelItems:
                    if(str(soup_vipLabelItem.find('span').string).replace('\n','').replace(' ','') == webName):
                        ...
                        if(soup_vipLabelItem.find('input').get('value') == webId):
                            ...
                            self.res_log = WriteDebugLog(self.res_log, print_tab+'- FAIL get webId: '+soup_vipLabelItem.find('input').get('value')+' != '+webId)
                    else:
                        result_list2.append('FAIL')
