...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Portal] API POST login WebMaster account (web)'
        self.CaseNote= ''
        self.TestCaseID= '418'
    ...
    def run(self, copy_result):
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Portal] API POST login WebMaster account (web)'+'\n')
        ...
        self.res_log, result = AdminLogin(self.res_log, 'WebMaster', self.filename, 'Web')
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Portal] API POST login WebMaster account (web)')
