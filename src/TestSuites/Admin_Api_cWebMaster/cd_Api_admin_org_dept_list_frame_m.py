...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST org. dept. list frame m'
        self.CaseNote= ''
        self.TestCaseID= '549'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. dept. list frame m'+'\n')
        print_tab = '\n\t\t'
        try:
            ...
            if(return_value):
                ...
                for soup_li in soup_lis:
                    ...
                    for soup_div in soup_divs:
                        if(agent_level4_dept in str(soup_div.text).replace('\n','').replace(' ','')): 
                            result_list2.append('PASS')
                            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get depname: '+agent_level4_dept)
                            agent_level4_dept_id = soup_li.find('i', id=True).get('id')
                            ReplaceIniValue('setting', 'API.agent_level4_dept_id', agent_level4_dept_id)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_org_dept_list_frame_m... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. dept. list frame m')
