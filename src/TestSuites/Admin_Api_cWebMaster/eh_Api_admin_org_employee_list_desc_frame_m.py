...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST org. employee list desc. frame m'
        self.CaseNote= ''
        self.TestCaseID= '567'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. employee list desc. frame m'+'\n')
        print_tab = '\n\t\t'
        try:
            ...
            ReplaceIniValue('admin_org_employee_list_desc_frame', 'API.depId', agent_level4_dept_id)
            ReplaceIniValue('admin_org_employee_list_desc_frame', 'API.titleId', agent_level4_dept_title_id)
            ReplaceIniValue('admin_org_employee_list_desc_frame', 'API.userAcc', agent_level4_dept_employee)
            ...
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_org_employee_list_desc_frame', False, False)
            ...
            if(return_value):
                ...
                if(user_Id == agent_level4_dept_employee_id):
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_org_employee_list_desc_frame_m... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. employee list desc. frame m')
