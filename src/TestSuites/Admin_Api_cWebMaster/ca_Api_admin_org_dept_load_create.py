...
class TestCase(BaseCase):
    ...
    def run(self, copy_result):
        ...
        print_tab = '\n\t\t'
        try:
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_org_dept_load_create', False, False)
            ...
            if(return_value):
                ...
                new_json = ReplaceAllJsonValue(response_data, '', '1').replace("'",'"').replace(' ','')
                ReplaceIniValue('admin_org_dept_create', 'API.permission', new_json)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_org_dept_load_create... %s' % str(e))
