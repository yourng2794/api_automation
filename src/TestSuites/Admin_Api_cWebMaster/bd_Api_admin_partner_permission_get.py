...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST partner permission get'
        self.CaseNote= ''
        self.TestCaseID= '449'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner permission get'+'\n')
        print_tab = '\n\t\t'
        try:
            agent_id_level4 = IniValue('setting', 'API.agent_id_level4')
            ReplaceIniValue('admin_partner_permission_get', 'API.ptParentId', agent_id_level4)
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_partner_permission_get', False, False)
            ...
            if(return_value):
                ...
                web_id = GetJsonValue(response_data, 'web_site|0|web_id')
                result_list.append('PASS')
                if(web_id== webId):
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get web_id: '+web_id)
                    result_list2.append('PASS')
                else:
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get web_id: '+web_id+' != '+webId)
                    result_list2.append('FAIL')
                result_list2.append(JsonValueCompare(response_data, 'permission|og', '0'))
                result_list2.append(JsonValueCompare(response_data, 'permission|op', '0'))
                result_list2.append(JsonValueCompare(response_data, 'permission|payment', '1'))
                result_list2.append(JsonValueCompare(response_data, 'permission|pro_e', '0'))
                result_list2.append(JsonValueCompare(response_data, 'permission|pt_player_level', '0'))
                result_list2.append(JsonValueCompare(response_data, 'permission|ws', '0'))
                result_list2.append(JsonValueCompare(response_data, 'permission|sb_e', '0'))
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_partner_permission_get... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST partner permission get')
