...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer promo web frame'
        self.CaseNote= ''
        self.TestCaseID= '489'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo web frame'+'\n')
        ...
        try:
            ...
            ReplaceIniValue('admin_webPlayer_promo_web_frame', 'API.promoid', promoid)
            ReplaceIniValue('admin_webPlayer_promo_web_frame', 'API.ptid', web_master_id)
            ReplaceIniValue('admin_webPlayer_promo_web_frame', 'API.api_path', 'api/webPlayer/promo/web/frame')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_webPlayer_promo_web_frame', False, False)
            ...
            if(return_value):
                ...
                soup_span = soup.find("input", {"name":"website"})
                if(soup_span.get("id")=='websiteAll'):
                    result_list.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get websiteAll: '+soup_span.get("id"))
                else:
                    result_list.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get websiteAll: '+soup_span.get("id"))
            ...
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_promo_web_frame... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo web frame')
