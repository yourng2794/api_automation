...
class TestCase(BaseCase):
    ...
    def run(self, copy_result):
        ...
        try:
            ...
            ReplaceIniValue('admin_webPlayer_promo_edit_get', 'API.ptId', web_master_id)
...
#                     print(str(amount).replace(' ','').replace("'",'"'))
                ...
                if((webId == res_web_id) and (webName == res_web_name)):
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success compare (webId, webName): ('+res_web_id+' ,'+res_web_name+')')
                    result_list2.append('PASS')
                else:
                    result_list2.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail compare (webId, webName): ('+res_web_id+' ,'+res_web_name+')')
