...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST org. dept. edit'
        self.CaseNote= ''
        self.TestCaseID= '551'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. dept. edit'+'\n')
        print_tab = '\n\t\t'
        try:
            agent_level4_dept_id = IniValue('setting', 'API.agent_level4_dept_id')
            agent_level4_dept = IniValue('setting', 'API.agent_level4_dept')
            desc = IniValue('setting', 'API.myname')
            ReplaceIniValue('admin_org_dept_edit', 'API.depId', agent_level4_dept_id)
            ReplaceIniValue('admin_org_dept_edit', 'API.name', agent_level4_dept)
            ReplaceIniValue('admin_org_dept_edit', 'API.descrip', desc)
            ReplaceIniValue('admin_org_dept_edit', 'API.permission', '{}')
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_org_dept_edit', True, False)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_org_dept_edit... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. dept. edit')
