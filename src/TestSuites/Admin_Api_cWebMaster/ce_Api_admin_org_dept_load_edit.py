...
class TestCase(BaseCase):
    ...
    def __init__(self,MyTest):
        ...
        self.CaseTitle= '[Feature] API POST org. dept. load edit'
        self.CaseNote= ''
        self.TestCaseID= '550'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. dept. load edit'+'\n')
        print_tab = '\n\t\t'
        try:
            agent_level4_dept_id = IniValue('setting', 'API.agent_level4_dept_id')
            ReplaceIniValue('admin_org_dept_load_edit', 'API.depId', agent_level4_dept_id)
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_org_dept_load_edit', False, False)
            ...
            if(return_value):
                ...
                agent_level4_dept = IniValue('setting', 'API.agent_level4_dept')
                if(json_response.get('department').get('og_name') == agent_level4_dept):
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get depname: '+agent_level4_dept)
                else:
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get depname: '+json_response.get('department').get('og_name')+' != '+agent_level4_dept)
                
                result_compare = JsonValueCompare(response_data, 'permission|og', '1')
                result_list.append(result_compare)
                result_compare = JsonValueCompare(response_data, 'permission|op', '1')
                result_list.append(result_compare)
                result_compare = JsonValueCompare(response_data, 'permission|ot', '1')
                result_list.append(result_compare)
                result_compare = JsonValueCompare(response_data, 'permission|pd', '1')
                result_list.append(result_compare)
                result_compare = JsonValueCompare(response_data, 'permission|pt', '1')
                result_list.append(result_compare)
                result_compare = JsonValueCompare(response_data, 'permission|pyd', '1')
                result_list.append(result_compare)
                result_compare = JsonValueCompare(response_data, 'permission|pyf', '1')
                result_list.append(result_compare)
                result_compare = JsonValueCompare(response_data, 'permission|ws', '1')
                result_list.append(result_compare)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_org_dept_load_edit... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST org. dept. load edit')
