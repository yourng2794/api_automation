...
class TestCase(BaseCase):
    ...
    def run(self, copy_result):
        ...
        result_list = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Portal] POST user login API'+'\n')
        ...
        try:
            ...
            with open('session', 'wb') as f:
                pickle.dump(session_id.cookies, f)
            return_value, response_time, response_data = ApiResponse('\n\t\t', session_id, 'admin_login_code', False, True)
            imgdata = b64decode(response_data)
            filename = 'code.jpg'  # I assume you have a way of picking unique filenames
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, '\n\t\t- Exception: on admin_login_code... %s' % str(e))
...
#                 ReplaceIniValue('setting', 'TestEnv.session_id', s)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, '\n\t\t- Exception: on admin_login... %s' % str(e))
            
        finally:
            self.res_log = WriteDebugLog(self.res_log, '---')
        
        try:
            return_value2, response_time2, response_data2 = ApiResponse('\n\t\t', session_id, 'admin_logout', True, False)
            self.res_log = WriteDebugLog(self.res_log, '\n\t\t- Response data: '+ str(response_data2))
            if(return_value2):
                result_list.append('PASS')
                self.res_log = WriteDebugLog(self.res_log, '\n\t\t- API success (status and response are correct).')
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, '\n\t\t- Exception: on admin_logout... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Portal] POST user login API')
