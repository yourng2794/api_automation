...
class TestCase(BaseCase):
    ...
    def run(self, copy_result):
        ...
        try:
            ...
            ReplaceIniValue('admin_org_dept_create', 'API.name', agent_level4_dept)
            ReplaceIniValue('admin_org_dept_create', 'API.descrip', desc)
            return_value, response_time, response_data, self.res_log = ApiResponse(print_tab, self.res_log, 'admin_org_dept_create', True, False)
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_org_dept_create... %s' % str(e))
