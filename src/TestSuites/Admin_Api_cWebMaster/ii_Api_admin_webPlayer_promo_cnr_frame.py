...
class TestCase(BaseCase):
    ...
    def __init__(self, MyTest):
        ...
        self.CaseTitle= '[Feature] API POST webPlayer promo cnr frame'
        self.CaseNote= ''
        self.TestCaseID= '491'
    ...
    def run(self, copy_result):
        ...
        result_list = []
        result_list2 = []
        self.res_log = WriteDebugLog(self.res_log, '\n\tStart >>'+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo cnr frame'+'\n')
        ...
        try:
            ...
            ReplaceIniValue('admin_webPlayer_promo_cnr_frame', 'API.ptid', web_master_id)
            ReplaceIniValue('admin_webPlayer_promo_cnr_frame', 'API.api_path', 'api/webPlayer/promo/cnr/frame')
            ...
            if(return_value):
                ...
                conditionReg = soup.find("input", {"name":"conditionReg"})
                conditionLogin = soup.find("input", {"name":"conditionLogin"})
                conditionTrunover = soup.find("input", {"name":"conditionTrunover"})
                ...
                if(conditionReg.get("value") == str(condition_reg)):
                    result_list2.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get conditionReg: '+conditionReg.get("value"))
                else:
                    result_list2.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get conditionReg: '+conditionReg.get("value")+ ' != '+ str(condition_reg))
                
                if(conditionLogin.get("value") == str(condition_login)):
                    result_list2.append('PASS')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Success get conditionLogin: '+conditionLogin.get("value"))
                else:
                    result_list2.append('FAIL')
                    self.res_log = WriteDebugLog(self.res_log, print_tab+'- Fail get conditionLogin: '+conditionLogin.get("value")+ ' != '+ str(condition_login))
            ...
            result_list.append('FAIL')
            self.res_log = WriteDebugLog(self.res_log, print_tab+'- Exception: on admin_webPlayer_promo_cnr_frame... %s' % str(e))
...
#         WriteLog('\t'+'Test Result: ' + self.TestResult)
        ...
        self.res_log = WriteDebugLog(self.res_log, '\n\n\tEnd << '+datetime.today().strftime("%m月%d日%H時%M分%S秒")+' [Feature] API POST webPlayer promo cnr frame')
